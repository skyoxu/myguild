...
namespace GuildSystem {
    ...
    getBonuses(): FacilityBonus[] {
      ...
      return config.bonuses.map(bonus => ({
        ...
        value:
          bonus.baseValue *
          this.level *
          (1 + bonus.scalingFactor * (this.level - 1)),
      }));
    }
  ...
  export class GuildLevelSystem {
    ...
    private static readonly EXP_FORMULA = (level: number) =>
      Math.floor(1000 * Math.pow(1.2, level));
    ...
    private static unlockFeatures(guild: Guild, level: number): void {
      switch (level) {
        ...
        case 20:
          // 解锁训练场
          guild.facilities.set(
            'training',
            new GuildFacility(FacilityType.TRAINING_GROUND)
          );
          break;
        case 30:
          // 解锁研究院
          guild.facilities.set(
            'research',
            new GuildFacility(FacilityType.RESEARCH_CENTER)
          );
          break;
      }
    }
  ...
  export class PermissionManager {
    ...
    private initializeDefaultRoles(): void {
      ...
      this.roles.set(GuildRole.TRIAL, {
        ...
        permissions: new Set([Permission.CHAT, Permission.VIEW_INFO]),
        canManageRoles: false,
        canKickMembers: false,
        canInviteMembers: false,
        canManageResources: false,
...
namespace RecruitmentSystem {
  ...
  export class RecruitmentManager {
    ...
    async searchCandidates(filters: RecruitmentFilters): Promise<Candidate[]> {
      ...
      if (filters.intimacyLevelFilter > 0) {
        candidates = this.applyIntimacyFilter(
          candidates,
          filters.intimacyLevelFilter
        );
      }
      ...
      if (filters.legendaryOnly) {
        candidates = candidates.filter(
          c => c.rarity === MemberRarity.LEGENDARY
        );
      }
    ...
    private aiRecommender = new (class AIRecommender {
      async recommend(filters: RecruitmentFilters): Promise<Candidate[]> {
        ...
        for (const candidate of recommendations) {
          candidate.matchScore = this.calculateMatchScore(
            candidate,
            guildNeeds
          );
        }
      ...
      private analyzeGuildNeeds(): GuildNeed[] {
        ...
        // 分析传奇成员需求
        const legendaryCount = guild.members.filter(
          m => m.rarity === MemberRarity.LEGENDARY
        ).length;
        if (legendaryCount < 3) {
          needs.push({
    ...
    private generateLegendaryCandidate(
      event: LegendaryRecruitmentEvent
    ): Candidate {
      const legendary: Candidate = {
        ...
        legendaryAbilities: this.generateLegendaryAbilities(
          event.legendaryType
        ),
  ...
  export class MemberManager {
    ...
    private updateFatigue(member: GuildMember, deltaTime: number): void {
      ...
      // 自然恢复
      member.fatigue = Math.max(
        0,
        member.fatigue - fatigueRecoveryRate * deltaTime
      );
    ...
    private updateSatisfaction(member: GuildMember, deltaTime: number): void {
      ...
      member.satisfaction = Math.max(
        0,
        Math.min(100, member.satisfaction + satisfactionChange * deltaTime)
      );
    }
    ...
    private checkAttritionRisk(member: GuildMember): void {
      ...
      const attritionProbability =
        this.calculateAttritionProbability(riskFactors);
  ...
  export class ActivityManager {
    ...
    private autoOrganizer = new (class AutoOrganizer {
      ...
      private generateOptimalComposition(
        ...
        // 优先选择传奇成员
        const legendaryMembers = availableMembers.filter(
          m => m.rarity === MemberRarity.LEGENDARY
        );
      ...
      private selectTactics(
        ...
        const availableTactics = TacticsLibrary.getAvailableTactics(
          activity.type
        );
        ...
        if (composition.hasLegendaryMember(LegendaryType.STRATEGIC_GENIUS)) {
          // 战术天才提供额外战术位
          const bonusTactic = availableTactics.find(
            t => t.category === TacticCategory.FORMATION_ENHANCEMENT
          );
    ...
    private rewardDistributor = new (class RewardDistributor {
      async distribute(
        ...
        for (const participant of participants) {
          const personalRewards = this.getPersonalRewards(
            participant,
            rewards,
            contributions
          );
  ...
  export class CombatEngine {
    ...
    private formulaCalculator = new (class FormulaCalculator {
      ...
      calculateDamage(
        ...
        // 基础伤害
        let damage =
          skill.baseDamage + attacker.attackPower * skill.powerScaling;
        ...
        // 属性克制
        const elementalBonus = this.getElementalBonus(
          skill.element,
    ...
    static MatchmakingSystem = class {
      ...
      private createMatch(
        team1: MatchmakingEntry,
        team2: MatchmakingEntry
      ): void {
