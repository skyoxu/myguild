...
> **目的**: 建立 Electron 应用的全面安全基线，覆盖进程隔离、IPC 安全、供应链防护等关键维度，确保 ${PRODUCT_NAME} 在桌面环境下的安全运行。
...
```mermaid
C4Container
    title Security Containers for ${PRODUCT_NAME}
...
```mermaid
C4Container
    title Multi-Process Security Model for ${PRODUCT_NAME}
...
// 确保配置类型安全
export type ValidSecurityConfig = SecurityConfigValidator<typeof MANDATORY_SECURITY_CONFIG>;
```
...
export function verifySecurityConfig(configPath) {
  ...
  // 检查必需配置
  for (const [key, expectedValue] of Object.entries(SECURITY_REQUIREMENTS.mandatory)) {
  ...
  // 检查禁止配置
  for (const [key, forbiddenValue] of Object.entries(SECURITY_REQUIREMENTS.forbidden)) {
    const actualValue = config.webPreferences?.[key];
...
export class SecurityConfigAuditor {
  ...
  public static auditBrowserWindow(window: BrowserWindow): SecurityAuditResult {
    ...
    if (webPrefs.webSecurity === false) {
      warnings.push('webSecurity is disabled - should only be used in development');
    }
...
export class CspManagerV2 {
  ...
  private readonly nonceStore = new Map<string, { nonce: string; timestamp: number }>();
  ...
  getCSPHeader(env: 'development' | 'production', requestId?: string): string {
    ...
    if (env === 'development') {
      ...
      return [
        ...
        "connect-src 'self' https://api.${PRODUCT_DOMAIN} wss://api.${PRODUCT_DOMAIN} https://sentry.io https://*.ingest.sentry.io ws://localhost:* wss://localhost:* ws://127.0.0.1:* wss://127.0.0.1:*",
    ...
    const productionCsp = [
      ...
      "connect-src 'self' https://api.${PRODUCT_DOMAIN} wss://api.${PRODUCT_DOMAIN} https://sentry.io https://*.ingest.sentry.io", // 与ADR-0002保持一致
...
const productionCSPExample = `
  ...
  img-src 'self' data: https: blob:;
  font-src 'self' data:;
  connect-src 'self' https://api.\${PRODUCT_DOMAIN} wss://api.\${PRODUCT_DOMAIN} https://sentry.io https://*.ingest.sentry.io;
  object-src 'none';
  frame-ancestors 'none';
  base-uri 'self';
  form-action 'self';
  ...
  ses.webRequest.onHeadersReceived((details, callback) => {
    ...
    callback({
      responseHeaders: {
        ...
        'Content-Security-Policy': [cspManager.getCSPHeader(environment, requestId)],
        // 基础安全标头
...
<meta http-equiv="Content-Security-Policy" content="
  ...
  connect-src 'self' https://api.${PRODUCT_DOMAIN} wss://api.${PRODUCT_DOMAIN} https://sentry.io https://*.ingest.sentry.io ws://localhost:* wss://localhost:*;
">
...
export function generateCspHashes(distDir: string): { scripts: string[], styles: string[] } {
  const scriptHashes = [];
  const styleHashes = [];
  ...
  // 提取内联脚本
  const scriptMatches = indexHtml.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/g);
  for (const match of scriptMatches) {
    if (match[1].trim()) {
      const hash = crypto.createHash('sha384').update(match[1]).digest('base64');
...
export function updateProductionCsp(distDir: string): void {
  ...
  console.log(`✅ CSP生产配置已更新，包含${scripts.length}个脚本hash和${styles.length}个样式hash`);
}
```
...
export class CspReporter {
  private violations: Array<{violatedDirective: string; blockedUri: string; timestamp: number}> = [];

  reportViolation(report: {violatedDirective: string; blockedUri: string}): void {
...
const ALLOWED_CHANNELS = [
  'app:getVersion', 'app:getPlatform', 'game:save', 'game:load', 
  'telemetry:track', 'security:reportViolation'
] as const;
...
contextBridge.exposeInMainWorld('${PRODUCT_SLUG}Api', secureApi);
...
// 注册处理器
registry.register('app:getVersion', () => process.env.npm_package_version || '0.0.0', 10);
registry.register('app:getPlatform', () => process.platform, 5);
registry.register('game:save', async (_, data) => {
  try {
    // 实现安全数据保存
    return {success: true};
  } catch (error) {
...
registry.register('telemetry:track', (_, event, props) => {
...
interface WindowWithApi extends Window {
  readonly ${PRODUCT_SLUG}Api?: import('../preload/security-bridge').ExposedApi;
}

class SecureApiClient {
  private api = (window as WindowWithApi).${PRODUCT_SLUG}Api;
...
test.describe('安全基线验证', () => {
  ...
  test('安全配置验证', async () => {
    ...
    const apiAvailable = await page.evaluate(() => 
      typeof (window as any).${PRODUCT_SLUG}Api !== 'undefined'
    );
  ...
  test('外部导航和窗口创建被阻止', async () => {
    const windowOpenBlocked = await page.evaluate(() => {
      try {
        return window.open('https://example.com', '_blank') === null;
      } catch {
        return true;
      }
    });
...
const SECURITY_PATTERNS = [
  ...
  { pattern: /nodeIntegration:\s*true/g, severity: 'critical', message: 'nodeIntegration enabled' },
  { pattern: /contextIsolation:\s*false/g, severity: 'critical', message: 'contextIsolation disabled' },
  { pattern: /sandbox:\s*false/g, severity: 'critical', message: 'sandbox disabled' }
];
...
const validateElectronConfig = async () => {
  ...
  for (const file of files) {
    ...
    if (content.includes('new BrowserWindow')) {
      const missing = Object.entries(patterns).filter(([_, pattern]) => !pattern.test(content));
      if (missing.length === 0) {
        log('passed', 'Electron安全配置验证通过');
      } else {
        missing.forEach(([desc]) => log('failed', `Missing: ${desc} in ${file}`));
      }
      return;
    }
  }
...
const validateCSP = () => {
  ...
  const required = ["default-src 'self'", "object-src 'none'", "child-src 'none'"];
