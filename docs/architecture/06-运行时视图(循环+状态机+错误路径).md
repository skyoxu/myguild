...
export class EventLoopMonitor extends EventEmitter {
  ...
  private checkPerformanceThresholds(metrics: EventLoopMetrics): void {
    ...
    if (utilization > 0.9) {
      ...
      console.warn(
        `⚠️ High Event Loop Utilization: ${(utilization * 100).toFixed(1)}%`
      );
    }
  }
...
export class ZeroCopyTransferManager {
  ...
  sendLargeDataToWorker(
    ...
    const strategy =
      ZeroCopyTransferManager.determineTransferStrategy(largeData);
    ...
    switch (strategy) {
      case 'transfer':
        ...
        console.log(
          `📦 Transferred ${largeData.byteLength}B via ArrayBuffer transfer`
        );
        break;
...
class ZeroCopyAIWorker {
  ...
  private initializeSharedResources(): void {
    ...
    if (typeof SharedArrayBuffer === 'undefined') {
      console.warn(
        '⚠️ SharedArrayBuffer not available, falling back to transfers'
      );
      return;
    }
  ...
  private async handleMessage(event: MessageEvent<ZeroCopyWorkerMessage>) {
    ...
    try {
      ...
      switch (type) {
        case 'pathfinding':
          result = await this.handleLargePathfinding(
            payload as LargePathfindingRequest,
            transferType
          );
          break;
        case 'map_generation':
  ...
  private async handleLargePathfinding(
    ...
    const path = this.runOptimizedAStar(
      start,
      target,
      obstacleArray,
      obstacleCount
    ...
    console.log(
      `🔍 Pathfinding completed with ${obstacleCount} obstacles (${transferType})`
    );
    return path;
  }

  /* 优化的返回策略 */
  private sendResultWithOptimalTransfer(
    id: string,
    type: string,
    result: any
  ): void {
...
export function useGameStateSync(eventBus: EventBus) {
  ...
  const syncLargeStateUpdate = useCallback(
    (newState: Partial<GameStateSnapshot>) => {
      startTransition(() => {
  ...
  const syncCriticalUpdate = useCallback(
    (update: Partial<GameStateSnapshot>) => {
...
export class FirstFramePerformanceMonitor {
  ...
  private calculateFirstFrameMetrics(): void {
    const { appStart, firstContentfulPaint, firstFrame, timeToInteractive } =
      this.metrics;

    if (appStart && firstFrame) {
      this.metrics.totalStartupTime = firstFrame - appStart;
      this.metrics.renderingTime =
        firstFrame - (firstContentfulPaint || appStart);
    }

    // 计算FPS稳定性
    if (this.fpsSamples.length > 0) {
      const avgFPS =
        this.fpsSamples.reduce((sum, fps) => sum + fps, 0) /
        this.fpsSamples.length;
      const variance =
        this.fpsSamples.reduce(
          (sum, fps) => sum + Math.pow(fps - avgFPS, 2),
          0
        ) / this.fpsSamples.length;
  ...
  private calculatePerformanceGrade(): 'excellent' | 'good' | 'fair' | 'poor' {
    ...
    const interactiveScore =
      timeToInteractive && timeToInteractive < 2000
        ? 3
        : timeToInteractive && timeToInteractive < 3000
          ? 2
          : timeToInteractive && timeToInteractive < 4000
            ? 1
            : 0;
...
export class TimeStepController {
  ...
  update(currentTime: number): GameLoopResult {
    ...
    this.accumulator += Math.min(
      frameTime,
      TimeStepController.MAX_ACCUMULATED_TIME
    );
...
export class PhaserTimeStepIntegration {
  constructor(
    ...
    private controller: TimeStepController
  ) {
  ...
  update(time: number, delta: number): void {
    ...
    try {
      ...
      const remainingBudget =
        this.performanceBudget - (performance.now() - updateStartTime);
...
export class EnhancedAppStateMachine extends StateMachine<
  AppContext,
  AppState
> {
      ...
      onEnter: async context => {
        ...
        context.sceneManager.scenes.forEach(scene => {
          if (
            ...
            !scene.sys.settings.data?.isPauseExempt
          ) {
            scene.scene.pause();
            // 保持系统更新但禁用游戏逻辑
...
export abstract class StateGuard<TContext> {
  ...
  abstract evaluate(
    guardContext: GuardContext<TContext>
...
export class ResourceLoadingGuard extends StateGuard<AppContext> {
  ...
  onGuardFailed(
    guardContext: GuardContext<AppContext>,
    result: GuardResult
  ): void {
    console.warn(
      `🛡️ 状态转换被阻止: ${guardContext.currentState} -> ${guardContext.targetState}`
    );
...
> extends StateMachine<TContext, TState> {
  ...
  private async executeTransition(
    request: TransitionRequest<TState>
  ): Promise<boolean> {
...
export class ConcurrentStateManager<TState extends string> {
  ...
  private detectConflicts(
    initiatorSystemId: string,
    newState: TState
  ): StateConflict<TState>[] {
...
export class DynamicBudgetManager {
  private static readonly TARGET_FPS = 60;
  private static readonly TARGET_FRAME_TIME =
    1000 / DynamicBudgetManager.TARGET_FPS; // 16.67ms
  ...
  private onFrameComplete(time: number, delta: number): void {
    const frameTime =
      this.phaserGame.loop.actualFps > 0
        ? 1000 / this.phaserGame.loop.actualFps
        : DynamicBudgetManager.TARGET_FRAME_TIME;
  ...
  private updatePerformanceMetrics(): void {
    ...
    // 计算负载因子 (当前使用时间 / 目标时间)
    this.performanceMetrics.loadFactor =
      avgFrameTime / DynamicBudgetManager.TARGET_FRAME_TIME;
  }
  ...
  getRemainingBudget(usedTime: number): number {
    const totalBudget = Object.values(this.currentBudgetAllocations).reduce(
  ...
  // 检查预算超支
  isBudgetExceeded(
    component: keyof typeof this.currentBudgetAllocations,
    usedTime: number
  ): boolean {
...
export class TimeSliceScheduler {
  ...
  scheduleTask(
    task: Omit<ScheduledTask, 'id' | 'createdAt' | 'attempts'>
  ): string {
  ...
  private executeAtomicTask(
    ...
    if (executionTime > task.maxExecutionTime) {
      console.warn(
        `原子任务执行超时: ${task.id} (${executionTime.toFixed(2)}ms > ${task.maxExecutionTime}ms)`
      );
    }
  ...
  private calculateAverageExecutionTime(): number {
    ...
    const totalTime = Array.from(this.completedTasks.values()).reduce(
...
export class WorkerPoolManager {
  ...
  private handleTaskComplete(
    ...
    console.debug(
      `任务完成: ${taskId} (Worker: ${workerId}, 耗时: ${executionTime}ms)`
    );
...
export class BackpressureController {
  ...
  private collectMetrics(): BackpressureMetrics {
    ...
    const metrics: BackpressureMetrics = {
      ...
      level: this.calculateBackpressureLevel(
        performanceReport,
        schedulerStatus,
        workerStatus
      ),
  ...
  private calculateBackpressureLevel(
    ...
    // Worker利用率评分 (0-1分)
    if (workerStatus.busyWorkers / Math.max(1, workerStatus.totalWorkers) > 0.9)
      score += 1;
  ...
  private updateBackpressureLevel(metrics: BackpressureMetrics): void {
    ...
    if (newLevel !== this.currentLevel) {
      console.log(
        `背压级别变化: ${BackpressureLevel[this.currentLevel]} -> ${BackpressureLevel[newLevel]}`
      );
  ...
  private applyDegradationStrategies(): void {
    // 应用当前级别的降级策略
    const currentActions = this.degradationActions.filter(
      action =>
    ...
    // 恢复不再需要的降级策略
    const actionsToRevert = this.degradationActions.filter(
      action =>
        action.level > this.currentLevel &&
        action.reversible &&
        this.activeGradations.has(action.action)
  ...
  private executeGradationAction(
    ...
    try {
      switch (action.action) {
        case 'defer_ai_tasks':
          this.scheduler.setMinPriority(
            apply ? TaskPriority.MEDIUM : TaskPriority.LOW
          );
          break;
  ...
  private calculateTrend(): 'improving' | 'stable' | 'deteriorating' {
    ...
    const recentAvgFPS =
      recent.reduce((sum, m) => sum + m.averageFPS, 0) / recent.length;
    const earlierAvgFPS =
      earlier.reduce((sum, m) => sum + m.averageFPS, 0) / earlier.length;
...
export class PhaserTimeStepIntegration {
  ...
  private onPreStep(time: number, delta: number): void {
    ...
    if (this.stats.frameTimeHistory.length > 0) {
      const lastFrameTime =
        this.stats.frameTimeHistory[this.stats.frameTimeHistory.length - 1];
  ...
  private detectPerformanceAnomalies(time: number, delta: number): void {
    ...
    if (delta > this.config.maxDelta * 0.8) {
      console.warn(
        `性能尖峰检测: delta=${delta.toFixed(2)}ms (阈值: ${this.config.maxDelta}ms)`
      );
    ...
    if (avgRecentFPS < this.config.targetFPS * 0.8) {
      console.warn(
        `FPS下降检测: ${avgRecentFPS.toFixed(1)}FPS (目标: ${this.config.targetFPS}FPS)`
      );
      ...
      this.phaserGame.events.emit('performance:fps_drop_detected', {
        ...
        targetFPS: this.config.targetFPS,
        severity:
          avgRecentFPS < this.config.targetFPS * 0.6 ? 'critical' : 'warning',
      });
    }
  }
  ...
  private logPerformanceMetrics(): void {
    const avgFPS =
      this.stats.fpsHistory.length > 0
        ? this.stats.fpsHistory.reduce((sum, fps) => sum + fps, 0) /
          this.stats.fpsHistory.length
        : 0;

    const avgDelta =
      this.stats.deltaHistory.length > 0
        ? this.stats.deltaHistory.reduce((sum, delta) => sum + delta, 0) /
          this.stats.deltaHistory.length
        : 0;

    console.log(
      `性能指标 [帧#${this.stats.frameCount}]: ` +
  ...
  generatePerformanceReport(): TimeStepPerformanceReport {
    ...
    // 计算帧时间的P95值
    const sortedFrameTimes = [...this.stats.frameTimeHistory].sort(
      (a, b) => a - b
    );
    const p95Index = Math.floor(sortedFrameTimes.length * 0.95);
    const p95FrameTime =
      sortedFrameTimes[p95Index] || this.stats.averageFrameTime;
  ...
  private calculateDeltaVariation(): number {
    ...
    const mean =
      ...
      this.stats.deltaHistory.length;
    const variance =
      this.stats.deltaHistory.reduce(
        (sum, delta) => sum + Math.pow(delta - mean, 2),
        0
      ) / this.stats.deltaHistory.length;
  ...
  private calculateFrameStability(): number {
    ...
    const mean =
      ...
      this.stats.fpsHistory.length;
    const variance =
      this.stats.fpsHistory.reduce(
        (sum, fps) => sum + Math.pow(fps - mean, 2),
        0
      ) / this.stats.fpsHistory.length;
    const stdDev = Math.sqrt(variance);
  ...
  private identifyPerformanceIssues(
    ...
    if (fps < this.config.targetFPS * 0.8) {
      issues.push(
        `FPS过低: ${fps.toFixed(1)} < ${(this.config.targetFPS * 0.8).toFixed(1)}`
      );
    }
    ...
    if (this.stats.maxFrameTime > this.config.maxDelta) {
      issues.push(
        `存在严重的帧时间尖峰: ${this.stats.maxFrameTime.toFixed(2)}ms`
      );
    }
...
/* 窗口创建错误监控 */
export function monitorWindowCreation(
  ...
  window.webContents.on('crashed', event => {
    Sentry.withScope(scope => {
      ...
      Sentry.captureException(
        new Error(`Renderer process crashed: ${windowName}`)
      );
    });
  });
...
export abstract class BaseScene extends Phaser.Scene {
  ...
  update(time: number, delta: number): void {
    try {
      ...
      this.errorHandler.captureGameError(
        error as Error,
        this.buildErrorContext(time, delta)
      );
...
export class ReleaseHealthMonitor {
  ...
  private calculateUserEngagement(): number {
    ...
    const actionsPerMinute =
      this.currentSession.userActions / Math.max(1, sessionMinutes);
    ...
    const checkFPS = () => {
      ...
      if (currentTime - lastFrameTime >= 1000) {
        ...
        if (fps < 30) {
          ...
          this.healthMetrics.performanceScore = Math.max(
            0,
            this.healthMetrics.performanceScore - 5
          );
...
test.describe('应用启动烟雾测试', () => {
  ...
  test('优雅关闭流程验证', async () => {
    ...
    await window.evaluate(() => {
      // 验证事件监听器清理
      const eventCount = Object.keys(
        (window as any).eventBus?.listeners || {}
      ).length;
      return eventCount;
    });
    ...
    // 验证进程已终止
    const processes = await electronApp
      .evaluate(({ app }) => {
        return app.isReady();
      })
      .catch(() => false);
...
test.describe('核心功能烟雾测试', () => {
  ...
  test('React-Phaser通信机制验证', async () => {
    ...
    const eventBusStatus = await window.evaluate(() => {
      return {
        ...
        listenerCount: Object.keys((window as any).eventBus?.listeners || {})
          .length,
      };
    });
    ...
    await expect(window.locator('[data-testid="score-display"]')).toContainText(
      '1000'
    );
  ...
  test('数据持久化功能验证', async () => {
    ...
    const dbPerformance = await window.evaluate(async () => {
      ...
      for (let i = 0; i < 100; i++) {
        operations.push(
          (window as any).database?.saveGameState({
            id: `test-${i}`,
  ...
  test('AI Worker计算能力验证', async () => {
    ...
    const concurrentTasks = await window.evaluate(async () => {
      ...
      for (let i = 0; i < 5; i++) {
        tasks.push(
          (window as any).aiWorker?.execute({
            type: 'parallel_test',
            data: { taskId: i },
          })
        );
...
test.describe('FPS性能监控测试', () => {
  ...
  test('高负载场景FPS压力测试', async () => {
    ...
    const stressTestFPS = await window.evaluate(() => {
      ...
      const stressMonitor = () => {
        ...
        if (currentTime - lastTime >= 1000) {
          ...
          const objectCount =
            (window as any).game?.scene?.getActiveScene()?.children.length || 0;
    ...
    // 4. 分析压力测试结果
    const avgStressFPS =
      stressTestFPS.reduce((a, b) => a + b.fps, 0) / stressTestFPS.length;
    const fpsDropRate =
      stressTestFPS.filter(m => m.fps < 30).length / stressTestFPS.length;
...
test.describe('webPreferences安全配置验证', () => {
  ...
  test('预加载脚本安全验证', async () => {
    ...
    expect(Object.values(preloadStatus.sensitiveAPIsBlocked)).not.toContain(
      false
    );
    ...
    const ipcSecurity = await window.evaluate(async () => {
      try {
        ...
        // 尝试调用未暴露的危险方法（应该失败）
        const dangerousResult = await (window as any).electronAPI
          .invoke('fs-read-file', '/etc/passwd')
          .catch(() => 'BLOCKED');
  ...
  test('沙箱模式验证', async () => {
    ...
    const sandboxLimitations = await window.evaluate(() => {
      return {
        ...
        // 子进程创建
        childProcessBlocked:
          typeof (window as any).child_process === 'undefined',
  ...
  test('敏感信息泄露防护验证', async () => {
    ...
    const dataFiltering = await window.evaluate(() => {
      ...
      return {
        originalContainsSensitive:
          JSON.stringify(testData).includes('secret123'),
        filteredContainsSensitive:
          JSON.stringify(filteredData).includes('secret123'),
        filteredData,
      };
    });
    ...
    const urlCleaning = await window.evaluate(() => {
      const sensitiveUrl =
        'https://api.example.com/data?token=secret123&user=test&password=hidden';
      ...
      return {
        ...
        cleanedUrl: cleanUrl,
        containsSensitive:
          cleanUrl.includes('secret123') || cleanUrl.includes('hidden'),
      };
    });
...
export class PerformanceMonitor {
  ...
  private checkThreshold(metric: PerformanceMetric): void {
    ...
    if (threshold.min && value < threshold.min) {
      ...
      threshold.target &&
      Math.abs(value - threshold.target) > threshold.target * 0.2
    ) {
      status = 'OFF_TARGET';
  ...
  private getTopPerformingMetrics(): Array<{
    ...
    this.thresholds.forEach((threshold, name) => {
      ...
      const deviation =
        Math.abs(stats.avg - threshold.target) / threshold.target;
  ...
  private generateRecommendations(): string[] {
    ...
    if (fpsViolations.length > 0) {
      recommendations.push(
        '考虑优化渲染性能：减少复杂动画、优化纹理使用、实现对象池'
      );
    }

    // 内存相关建议
    const memoryViolations = violations.filter(v =>
      v.metric.includes('memory')
    );
    if (memoryViolations.length > 0) {
      recommendations.push(
        '考虑优化内存使用：及时清理未使用资源、优化纹理大小、实现资源缓存策略'
      );
    }

    // 启动时间建议
    const startupViolations = violations.filter(v =>
      v.metric.includes('startup')
    );
    if (startupViolations.length > 0) {
      recommendations.push(
        '考虑优化启动性能：延迟加载非关键资源、预编译资源、优化依赖加载'
      );
    }

    // 网络相关建议
    const networkViolations = violations.filter(
      v => v.metric.includes('api') || v.metric.includes('network')
    );
    if (networkViolations.length > 0) {
      recommendations.push(
        '考虑优化网络性能：实现请求缓存、优化API响应大小、使用连接池'
      );
    }
  ...
  setThreshold(
    name: string,
    threshold: Omit<PerformanceThreshold, 'name'>
  ): void {
...
export class RuntimeSecurityValidator extends EventEmitter {
  ...
  private generateSecurityRecommendations(
    ...
    if (!checks.contextIsolation)
      recommendations.push('立即启用contextIsolation以隔离渲染进程');
    if (!checks.nodeIntegration)
      recommendations.push('确保nodeIntegration已禁用');
    if (!checks.webSecurity)
      recommendations.push('启用webSecurity以强制执行同源策略');
    if (!checks.ipcWhitelist) recommendations.push('审查并清理未授权的IPC通道');
    if (!checks.preloadIntegrity)
      recommendations.push('验证预加载脚本完整性或更新基线哈希');
    if (!checks.cspPolicies)
      recommendations.push('实施严格的内容安全策略(CSP)');
  ...
  private updateSecurityMetrics(result: SecurityValidationResult): void {
    ...
    const criticalPenalty =
      result.violations.filter(v => v.severity === 'critical').length * 25;
    const highPenalty =
      result.violations.filter(v => v.severity === 'high').length * 15;
    const mediumPenalty =
      result.violations.filter(v => v.severity === 'medium').length * 10;
    const lowPenalty =
      result.violations.filter(v => v.severity === 'low').length * 5;
  ...
  /* API方法 */
  getLastValidationResult(): SecurityValidationResult | null {
    return this.lastValidation;
  }
  getSecurityMetrics(): Map<string, number> {
    return new Map(this.securityMetrics);
  }
  async triggerValidation(): Promise<SecurityValidationResult> {
    return this.performFullSecurityValidation();
  }
  ...
  test('contextIsolation隔离验证', async () => {
    ...
    const exposedAPIs = await window.evaluate(() => {
      return {
        ...
        exposedMethods: (window as any).electronAPI
    ...
    expect(exposedAPIs.exposedMethods).toEqual(
      expect.arrayContaining(['invoke', 'send', 'on'])
    );
  });
...
import {
  ...
  SecurityViolation,
} from '../security/runtime-security-validator';
...
export class SecurityMonitorIntegration {
  ...
  private async generateSecurityPerformanceCorrelation(): Promise<SecurityPerformanceCorrelation> {
    ...
    try {
      ...
      const correlations = this.analyzeSecurityPerformanceImpact(
        securityScore,
        performanceMetrics
      );
  ...
  /* 分析安全性能影响 */
  private analyzeSecurityPerformanceImpact(
    securityScore: number,
  ...
  private activateEmergencySecurityMode(): void {
    ...
    console.error(
      '🚨 EMERGENCY SECURITY MODE ACTIVATED - Critical security violations detected'
    );
  }
...
/* 导出全局实例 */
export const securityMonitorIntegration =
  SecurityMonitorIntegration.getInstance();
```
