...
| `SENTRY_TOKEN` | Sentry API Token | ✅ | sntrys\_**_ |
| `SENTRY_DSN` | Production DSN | ✅ | https://_**@sentry.io/**_ |
| `SENTRY_DSN_STAGING` | Staging DSN | ⚠️ | https://_**@sentry.io/\*\*\* |
...
describe('Sentry 渲染进程初始化', () => {
  ...
  it('应监控用户交互事件', async () => {
    ...
    // 验证交互被记录
    expect(
      vi.mocked(require('@sentry/electron/renderer').addBreadcrumb)
  ...
  it('应处理 React 错误边界', async () => {
    ...
    expect(
      vi.mocked(require('@sentry/electron/renderer').captureException)
    ).toHaveBeenCalledWith(
      testError,
      expect.objectContaining({
...
describe('可观测性系统集成测试', () => {
  ...
  it('应协调工作：错误监控 + 日志记录 + 性能监控', async () => {
    ...
    expect(
      require('@sentry/electron/main').captureException
