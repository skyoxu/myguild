...
class GuildTreasury {
  ...
  depositGold(amount: Money): GuildTreasury {
    return new GuildTreasury(this.gold.add(amount), new Map(this.resources));
  }
  ...
  withdrawResource(resourceType: string, amount: number): GuildTreasury {
    ...
    if (currentAmount < amount) {
      throw new Error(
        `Insufficient ${resourceType}: need ${amount}, have ${currentAmount}`
      );
    }
...
class GuildAggregate {
  ...
  recruitMember(
    ...
    this.membershipRegistry = this.membershipRegistry.addMember(
      characterId,
      role
    );
    this.incrementVersion();
  ...
  promoteMember(
    ...
    // 权限检查
    if (
      !this.membershipRegistry.hasPermission(promotedBy, GuildRole.VICE_LEADER)
    ) {
    ...
    this.membershipRegistry = this.membershipRegistry.updateMemberRole(
      characterId,
      newRole
    );
    this.incrementVersion();
...
class BattleAggregate {
  ...
  executeRound(): void {
    ...
    if (
      newAttackerPower === 0 ||
      newDefenderPower === 0 ||
      currentRound >= this.configuration.maxRounds
    ) {
      this.completeBattle();
    }
  ...
  private calculateRewards(
    ...
    if (winnerParty) {
      ...
      winnerParty.memberIds.forEach(memberId => {
        rewards.set(
          memberId,
          new BattleReward(baseExp, baseGold, new Map([['battle_token', 1]]))
        );
      });
    }
...
class DomainInvariantValidator {
  ...
  static validateGuildInvariants(guild: GuildAggregate): void {
    ...
    if (leaderCount !== 1) {
      throw new Error(
        `Guild must have exactly one leader, but found ${leaderCount}. Guild ID: ${guild.id.toString()}`
      );
    }
...
class AggregateBoundaryEnforcer {
  ...
  static enforceGuildOperationBoundary(
    ...
    if (!membership.hasPermission(requiredRole)) {
      throw new Error(
        `Operation '${operation}' requires role ${requiredRole} or higher`
      );
    }
  }
...
class BattleSystemService {
  ...
  private validateBattleParticipants(
    members: CharacterAggregate[],
    ...
    members.forEach(member => {
      ...
      if (!membership || !membership.guildId.equals(expectedGuildId)) {
        throw new Error(
          `Character ${member.id} is not a member of the expected guild`
        );
      }
    });
  }
...
class EconomicSystemService {
  ...
  async transferMoney(
    ...
    // 验证发送方余额
    if (
      !from.getWallet().isGreaterThan(amount) &&
      !from.getWallet().equals(amount)
    ) {
      throw new Error('Insufficient funds');
    }
  ...
  async distributeGuildResources(
    ...
    guild: GuildAggregate;
    distributions: Array<{ characterId: CharacterId; amount: number }>;
  }> {
    // 验证分配者权限
    AggregateBoundaryEnforcer.enforceGuildOperationBoundary(
      guild,
...
class QuestRewardWorkflow {
  ...
  async completeQuest(
    ...
    // 1. 任务系统上下文：完成任务
    const questRewards = await this.questService.completeQuest(
      characterId,
      questId
    );
    ...
    // 3. 发布领域事件，触发跨上下文通信
    const event = new QuestCompletedEvent(characterId, questId, {
...
class ContextMap {
  ...
  /** 定义上下文之间的关系 */
  private static readonly relationships = new Map<
    string,
    {
      downstream: string;
      upstream: string;
  ...
  >([
    [
      'quest_to_guild',
      {
        ...
        description:
          '任务系统发布QuestCompleted事件，公会管理系统监听并更新公会经验',
      },
    ],
    [
      'battle_to_guild',
      {
        ...
        relationship: ContextRelationship.UPSTREAM_DOWNSTREAM,
        description:
          '战斗系统发布BattleCompleted事件，公会管理系统监听并更新成员统计',
      },
    ],
    [
...
class BattleSystemACL {
  ...
  /** 将角色数据转换为战斗成员格式 */
  static adaptCharacterForBattle(
    character: CharacterAggregate
  ): BattleMemberView {
...
class EventProcessingOptimizer {
  ...
  static async processBatch(
    ...
    eventGroups.forEach((eventList, eventType) => {
      processingPromises.push(this.processEventGroup(eventList, eventBus));
    });
...
interface ICharacterRepository
  extends IRepository<CharacterAggregate, CharacterId> {
...
interface IEventStore extends SecondaryPort {
  ...
  /** 订阅所有事件 */
  subscribeToAll(handler: (event: StoredEvent) => Promise<void>): Promise<void>;
...
interface IAggregateRebuilder<TAggregateRoot> {
  /** 从事件流重建聚合 */
  rebuildFromEvents(events: DomainEvent[], snapshot?: any): TAggregateRoot;
...
/** 角色聚合重建器 */
class CharacterAggregateRebuilder
  implements IAggregateRebuilder<CharacterAggregate>
{
  rebuildFromEvents(events: DomainEvent[], snapshot?: any): CharacterAggregate {
  ...
  implements IRepository<TAggregateRoot, TId>
{
  ...
  async delete(id: TId): Promise<void> {
    ...
    throw new Error(
      'Deletion not supported in Event Sourcing. Use soft delete events instead.'
    );
  }
  ...
  async findAll(offset?: number, limit?: number): Promise<TAggregateRoot[]> {
    ...
    throw new Error(
      'findAll not efficiently supported in Event Sourcing. Use read models instead.'
    );
  }
  ...
  async count(): Promise<number> {
    // 同样，建议使用读模型
    throw new Error(
      'count not efficiently supported in Event Sourcing. Use read models instead.'
    );
  }
...
interface ILeaderboardService extends SecondaryPort {
  ...
  /** 获取实体在排行榜中的排名 */
  getRankOf(type: LeaderboardType, entityId: string): Promise<number | null>;
  ...
  /** 归档排行榜(用于赛季结算) */
  archiveLeaderboard(type: LeaderboardType, seasonId: string): Promise<void>;
}
```
...
interface IGuildQueryService extends SecondaryPort {
  ...
  /** 查询公会详细信息 */
  getGuildDetails(guildId: GuildId): Promise<GuildDetailsView | null>;
...
interface IBattleQueryService extends SecondaryPort {
  ...
  /** 查询战斗详细信息 */
  getBattleDetails(battleId: BattleId): Promise<BattleDetailsView | null>;
  ...
  implements ICharacterRepository
{
  ...
  async save(character: CharacterAggregate): Promise<void> {
    ...
    this.executeInTransaction(() => {
      const existsQuery = this.db.prepare(
      ...
      if (exists) {
        ...
        if (result.changes === 0) {
          throw new Error(
            'Optimistic lock failed: Character was modified by another process'
          );
        }
      } else {
        // 插入
...
class DIContainer {
  ...
  private checkCircularDependency(
    ...
    if (visited.has(token)) {
      throw new Error(
        `Circular dependency detected: ${Array.from(visited).join(' -> ')} -> ${token}`
      );
    }
...
class ContainerConfigurator {
  ...
  static configureDevelopment(container: DIContainer): void {
    ...
    // 简单的内存事件总线
    container.singleton(ServiceTokens.EVENT_BUS, InMemoryEventBus);

    container.singleton(ServiceTokens.EVENT_STORE, InMemoryEventStore);

    // Mock外部服务
    container.singleton(ServiceTokens.PAYMENT_SERVICE, MockPaymentService);
    ...
    // 工作单元
    container.singleton(ServiceTokens.UNIT_OF_WORK, InMemoryUnitOfWork);
  }
  ...
  static configureProduction(
    ...
    container.singleton(ServiceTokens.GUILD_REPOSITORY, SqliteGuildRepository, [
      ServiceTokens.DATABASE,
    ]);
    ...
    if (config.enableEventSourcing) {
      container.singleton(ServiceTokens.EVENT_STORE, SqliteEventStore, [
        ServiceTokens.DATABASE,
      ]);
    } else {
      container.singleton(ServiceTokens.EVENT_STORE, InMemoryEventStore);
    ...
    // 事件总线
    container.singleton(ServiceTokens.EVENT_BUS, DefaultEventBus, [
    ...
    // SQLite工作单元
    container.singleton(ServiceTokens.UNIT_OF_WORK, SqliteUnitOfWork, [
      ServiceTokens.DATABASE,
    ]);
  }
  ...
  static configureTest(container: DIContainer): void {
    ...
    container.transient(ServiceTokens.GUILD_REPOSITORY, MockGuildRepository);

    container.transient(ServiceTokens.BATTLE_REPOSITORY, MockBattleRepository);

    // 可控制的事件总线
    container.transient(ServiceTokens.EVENT_BUS, TestEventBus);

    container.transient(ServiceTokens.EVENT_STORE, TestEventStore);
    ...
    // 测试工作单元
    container.transient(ServiceTokens.UNIT_OF_WORK, TestUnitOfWork);
  }
}
...
abstract class RepositoryContractTest<TAggregateRoot, TId> extends TestBase {
  ...
  /** 修改聚合状态 - 子类实现 */
  protected abstract modifyAggregate(
...
/** 角色Repository契约测试 */
class CharacterRepositoryContractTest extends RepositoryContractTest<
  CharacterAggregate,
  CharacterId
> {
  protected getRepository(): ICharacterRepository {
    return this.getService<ICharacterRepository>(
      ServiceTokens.CHARACTER_REPOSITORY
    );
  }
...
class ControllablePaymentService implements IPaymentService {
  ...
  async verifyPayment(transactionId: string): Promise<PaymentResult> {
    ...
    const existing = this.paymentResults.find(
  ...
  async refund(
    ...
    const payment = this.paymentResults.find(
      p => p.transactionId === transactionId
    );
    if (!payment) {
      throw new Error(`Transaction ${transactionId} not found`);
...
class ControllableNotificationService implements INotificationService {
  ...
  async sendToCharacters(
    ...
    for (const characterId of characterIds) {
      const characterResults = await this.sendToCharacter(
        characterId,
        message,
        channels
      );
...
class ArchitectureValidator {
  ...
  static validatePortCompliance(portInstance: any): ValidationResult {
    ...
    // 检查端口类型是否有效
    if (
      portInstance.portType &&
      !['primary', 'secondary'].includes(portInstance.portType)
    ) {
...
class ArchitectureTestSuite {
  ...
  private static async testRepositoryImplementations(
    container: DIContainer
  ): Promise<void> {
    ...
    for (const token of repositoryTokens) {
      ...
      const result =
        ArchitectureValidator.validateRepositoryCompliance(repository);
      console.log(
        `  ${token}: ${result.isValid ? '✅' : '❌'} ${result.toString()}`
      );
    }
  }

  private static async testEventArchitecture(
    container: DIContainer
  ): Promise<void> {
    const eventBus = container.resolve<IEventBus>(ServiceTokens.EVENT_BUS);
    const eventStore = container.resolve<IEventStore>(
      ServiceTokens.EVENT_STORE
    );

    const result = ArchitectureValidator.validateEventDrivenArchitecture(
      eventBus,
      eventStore
    );
    console.log(
      `  Event Architecture: ${result.isValid ? '✅' : '❌'} ${result.toString()}`
    );
  }

  private static async testExternalServicePorts(
    container: DIContainer
  ): Promise<void> {
    const serviceTokens = [
      ServiceTokens.PAYMENT_SERVICE,
...
class PerformanceMetricsCollector {
  ...
  getPerformanceReport(): PerformanceReport {
    const operationReports = Array.from(this.metrics.values()).map(metric =>
      metric.getReport()
    );
...
class PerformanceMetric {
  ...
  getReport(): OperationReport {
    if (this.measurements.length === 0) {
      return new OperationReport(this.operationName, 0, 0, 0, 0, 0, 0, 100);
    }
    ...
    const averageDuration =
      durations.reduce((sum, d) => sum + d, 0) / totalCount;
...
function MonitorPerformance(operationName?: string) {
  return function (
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      return await collector.measureOperation(finalOperationName, () =>
...
class InMemoryStorageAdapter<TEntity, TId>
  implements StoragePort<TEntity, TId>
{
  ...
  async findMany(criteria: QueryCriteria<TEntity>): Promise<TEntity[]> {
    ...
    if (criteria.filters) {
      results = results.filter(entity =>
        Object.entries(criteria.filters!).every(
...
class StorageContainer {
  ...
  configure(config: StorageConfig): void {
    this.repositories.set(
      'character',
      StorageAdapterFactory.createCharacterRepository(config)
    );
    this.repositories.set(
      'guild',
      StorageAdapterFactory.createGuildRepository(config)
    );
    this.repositories.set(
      'battle',
      StorageAdapterFactory.createBattleRepository(config)
    );
  }
...
class DataPathValidator {
  ...
  static validateDatabasePath(path: string): ValidationResult {
    ...
    if (path.length > 260) {
      issues.push(
        'Database path exceeds Windows maximum path length (260 characters)'
      );
    }
...
class SQLiteConnectionManager {
  ...
  async initialize(config: Partial<SQLiteConfig> = {}): Promise<void> {
    ...
    try {
      ...
      console.log(
        `WAL mode: ${this.config.enableWAL ? 'enabled' : 'disabled'}`
      );
      console.log(
        `Busy timeout: ${this.config.busyTimeout}ms (并发写冲突处理)`
      );
    } catch (error) {
  ...
  private createIndexes(): void {
    ...
    // 公会相关索引
    this.database.exec(
      'CREATE INDEX IF NOT EXISTS idx_guilds_reputation ON guilds(reputation DESC)'
    );
    this.database.exec(
    ...
    this.database.exec(
      'CREATE INDEX IF NOT EXISTS idx_battles_attacker ON battles(attacker_guild_id)'
    );
    this.database.exec(
      'CREATE INDEX IF NOT EXISTS idx_battles_defender ON battles(defender_guild_id)'
    );
    this.database.exec(
  ...
  async createBackup(): Promise<string> {
    ...
    const backupPath =
      ElectronDataPathManager.getInstance().generateBackupPath();
...
class SQLiteStorageAdapter<TEntity, TId> implements StoragePort<TEntity, TId> {
  ...
  async transaction<T>(
    ...
    // 复杂事务操作需要特殊处理
    throw new Error(
      'Complex transactions not yet implemented in worker adapter'
    );
  }
  ...
  /** 构建查询SQL */
  private buildQuery(criteria: QueryCriteria<TEntity>): {
    sql: string;
    params: any[];
  } {
    ...
    if (criteria.filters) {
      const conditions = Object.entries(criteria.filters).map(
        ([key, value]) => {
          params.push(value);
          return `${key} = ?`;
        }
  ...
  private loadConfiguration(): EnvironmentConfig {
    ...
    try {
      const userConfigPath =
        ElectronDataPathManager.getInstance().getConfigPath();
      if (require('fs').existsSync(userConfigPath)) {
        const userConfig = JSON.parse(
          require('fs').readFileSync(userConfigPath, 'utf8')
        );
...
class ApplicationStorageInitializer {
  ...
  private static async validateStorage(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Storage validation passed: found ${guilds.length} existing guilds`
      );
    } catch (error) {
...
class MigrationExecutor {
  ...
  async migrate(
    ...
    try {
      ...
      if (
        ...
        compatibility.action === 'incompatible'
      ) {
      ...
      // 获取需要执行的迁移
      const migrations = this.registry.getMigrationsInRange(
        currentVersion,
        targetVersion
      );
      ...
      if (migrations.length === 0) {
        throw new Error(
          `No migrations found between version ${currentVersion} and ${targetVersion}`
        );
      }

      console.log(
        `Starting migration from version ${currentVersion} to ${targetVersion}`
      );
      console.log(
        `Migrations to execute: ${migrations.map(m => m.version).join(', ')}`
      );
      ...
      if (result.success) {
        ...
        console.log(
          `Migration completed successfully. Database is now at version ${targetVersion}`
        );
      }
  ...
  private async createBackup(customPath?: string): Promise<string> {
    const pathManager = ElectronDataPathManager.getInstance();
    const backupPath =
      customPath || pathManager.generateBackupPath('-before-migration');
  ...
  private async restoreFromBackup(backupPath: string): Promise<void> {
    ...
    // 复制备份文件到原位置
    const currentDbPath =
      ElectronDataPathManager.getInstance().getDatabasePath();
...
class Migration001_InitialSchema extends BaseMigration {
  ...
  up(db: Database.Database): void {
    ...
    // 创建基础索引
    this.createIndexIfNotExists(db, 'idx_characters_guild_id', 'characters', [
      'guild_id',
    ]);
    this.createIndexIfNotExists(db, 'idx_guilds_reputation', 'guilds', [
      'reputation DESC',
    ]);
  }
...
class Migration002_AddBattleSystem extends BaseMigration {
  ...
  up(db: Database.Database): void {
    ...
    // 创建索引
    this.createIndexIfNotExists(db, 'idx_battles_status', 'battles', [
      'status',
    ]);
    this.createIndexIfNotExists(db, 'idx_battles_attacker', 'battles', [
      'attacker_guild_id',
    ]);
    this.createIndexIfNotExists(db, 'idx_battles_defender', 'battles', [
      'defender_guild_id',
    ]);
...
class Migration004_RefactorCharacterAttributes extends BaseMigration {
  version = 4;
  description =
    'Refactor character attributes structure for better performance';
  ...
  up(db: Database.Database): void {
    ...
    // 迁移现有数据
    const characters = db
      .prepare('SELECT id, attributes FROM characters WHERE attributes != "{}"')
      .all();
    ...
    const updateTransaction = db.transaction((chars: any[]) => {
      for (const char of chars) {
        try {
          ...
          console.warn(
            `Failed to parse attributes for character ${char.id}, using defaults`
          );
  ...
  down(db: Database.Database): void {
    ...
    this.recreateTableWithoutColumns(db, 'characters', [
      'strength',
      'agility',
      'intelligence',
      'vitality',
  ...
  /** 重建表以移除指定列 */
  private recreateTableWithoutColumns(
    db: Database.Database,
    tableName: string,
    ...
    const tableInfo = db
      ...
      .all() as any[];
    const remainingColumns = tableInfo.filter(
      col => !columnsToRemove.includes(col.name)
    );
    ...
    console.log(
      `Recreated table ${tableName} without columns: ${columnsToRemove.join(', ')}`
    );
  }

  validate(db: Database.Database): boolean {
    return (
      this.columnExists(db, 'characters', 'strength') &&
...
class MigrationLoader {
  ...
  /** 从指定目录加载所有迁移脚本 */
  async loadMigrationsFromDirectory(
    migrationDir: string
  ): Promise<Migration[]> {
    ...
    try {
      ...
      console.log(
        `Loaded ${migrations.length} migrations from ${migrationDir}`
      );
      return migrations.sort((a, b) => a.version - b.version);
    } catch (error) {
      console.error(
        `Failed to read migration directory ${migrationDir}:`,
        error
      );
      return [];
    }
  ...
  /** 从单个文件加载迁移脚本 */
  private async loadMigrationFromFile(
    filePath: string
  ...
  /** 验证加载的迁移脚本 */
  private validateLoadedMigration(
    migration: Migration,
    filePath: string
  ): void {
    ...
    if (typeof migration.down !== 'function') {
      throw new Error(
        `Missing or invalid down method in migration ${filePath}`
      );
    }
  }
  ...
  getLoadedMigrationInfo(): MigrationModuleInfo[] {
    return Array.from(this.loadedModules.values()).sort(
...
class BuiltInMigrations {
  ...
  /** 按版本范围获取迁移脚本 */
  static getMigrationsByVersionRange(
    minVersion: number,
    maxVersion: number
  ): Migration[] {
...
class RollbackManager {
  ...
  createRollbackPlan(targetVersion: number): RollbackPlan {
    ...
    // 获取需要回滚的迁移
    const migrationsToRollback = this.registry.getMigrationsInRange(
      targetVersion,
      currentVersion
    );
    ...
    const estimatedDuration =
      this.estimateRollbackDuration(migrationsToRollback);
  ...
  async executeRollback(
    ...
    // 安全性检查
    if (
      plan.riskAssessment.riskLevel === 'critical' &&
    ...
    console.log(
      `Starting rollback from version ${plan.currentVersion} to ${plan.targetVersion}`
    );
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `Rollback completed successfully. Database is now at version ${options.targetVersion}`
        );
      }
  ...
  private assessRollbackRisk(
    ...
    for (const migration of migrations) {
      ...
      if (migration.type === 'data') {
        dataLossRisk = true;
        risks.push(
          `Migration ${migration.version} involves data transformation`
        );
      }
  ...
  private async performDryRun(
    ...
    for (const migration of plan.migrationsToRollback) {
      console.log(
        `Would rollback migration ${migration.version}: ${migration.description}`
      );

      // 检查回滚脚本的有效性
      if (typeof migration.down !== 'function') {
        throw new Error(
          `Migration ${migration.version} has no valid down method`
        );
      }
    }
  ...
  private async reimportProtectedData(
    ...
    try {
      const protectedData = JSON.parse(
        require('fs').readFileSync(exportPath, 'utf8')
      );
      ...
      for (const [tableName, rows] of Object.entries(protectedData)) {
        ...
        if (!this.tableExists(tableName)) {
          console.warn(
            `Table ${tableName} no longer exists, skipping data import`
          );
          continue;
        }
  ...
  /** 重新导入单个表的数据 */
  private async reimportTableData(
    tableName: string,
    rows: any[]
  ): Promise<void> {
...
class IndexManager {
  ...
  createIndex(definition: IndexDefinition): void {
    try {
      ...
      console.log(
        `Created index ${definition.name} on ${definition.tableName}(${definition.columns.join(', ')})`
      );
  ...
  generateIndexReport(): {
    ...
    const summary = {
      ...
      lowSelectivityIndexes: analyses.filter(a => a.stats.selectivity < 0.1)
        .length,
    };
  ...
  private getIndexDefinition(indexName: string): IndexDefinition | null {
    ...
    const columns = columnMatch
      ? columnMatch[1].split(',').map((col: string) => col.trim())
      : [];
  ...
  /** 计算索引选择性 */
  private calculateIndexSelectivity(indexInfo: IndexDefinition): {
    uniqueValues: number;
    ratio: number;
  } {
    try {
      ...
      console.warn(
        `Failed to calculate selectivity for index ${indexInfo.name}:`,
        error
      );
      return { uniqueValues: 0, ratio: 0 };
    }
  ...
  private analyzeIndexUsage(
    ...
    if (selectivity.ratio < 0.01) {
      ...
      reason =
        'Low selectivity (< 10%), consider optimizing or adding WHERE clause';
    }
...
class ConstraintManager {
  ...
  validateTableForeignKeys(tableName: string): ConstraintViolation[] {
    ...
    try {
      const fkViolations = this.database.pragma(
        `foreign_key_check(${tableName})`
      ) as any[];
      ...
      console.error(
        `Failed to validate foreign keys for table ${tableName}:`,
        error
      );
    }
  ...
  checkDatabaseIntegrity(): {
    ...
    try {
      ...
      const integrityResults = this.database.pragma(
        'integrity_check'
      ) as string[];
  ...
  getTableConstraints(tableName: string): ConstraintDefinition[] {
    ...
    try {
      // 获取表结构信息
      const tableInfo = this.database.pragma(
      ...
      for (const index of indexes) {
        if (index.unique) {
          const indexInfo = this.database.pragma(
        ...
        for (const violation of violations) {
          try {
            if (strategy === 'delete_orphans') {
              ...
              console.warn(
                `Manual intervention required for violation in ${violation.tableName}`
              );
              failed++;
            }
          } catch (error) {
            errors.push(
              `Failed to fix violation in ${violation.tableName}: ${error.message}`
            );
            failed++;
          }
        }
  ...
  generateConstraintReport(): {
    ...
    if (violationCount > 0) {
      recommendations.push(
        `Found ${violationCount} constraint violations that need attention`
      );
    }

    if (!this.getForeignKeyStatus()) {
      recommendations.push(
        'Consider enabling foreign key constraints for better data integrity'
      );
    }

    const tablesWithoutFK = tableDetails.filter(
      t => !t.constraints.some(c => c.type === 'foreign_key')
    );

    if (tablesWithoutFK.length > 0) {
      recommendations.push(
        `${tablesWithoutFK.length} tables have no foreign key constraints`
      );
    }
...
class DatabaseMigrationSystem {
  ...
  generateMigrationReport(): {
    ...
    if (migrationNeeded) {
      recommendations.push(
        `Database migration needed: ${version.current} → ${version.target}`
      );
    }
    ...
    if (health.indexReport.summary.unusedIndexes > 0) {
      recommendations.push(
        `${health.indexReport.summary.unusedIndexes} unused indexes found`
      );
    }
...
async function initializeGameDatabase(): Promise<void> {
  try {
    ...
    console.log(
      `Integrity: ${health.integrity.passed ? 'OK' : 'ISSUES FOUND'}`
    );
    ...
    if (health.compatibility.action === 'upgrade') {
      ...
      if (migrationResult.success) {
        console.log(
          `Migration completed successfully in ${migrationResult.executionTime}ms`
        );
        console.log(
          `Applied ${migrationResult.affectedMigrations.length} migrations`
        );
      } else {
...
class DatabaseMigrationValidator {
  ...
  async validateDatabaseMigrations(): Promise<AcceptanceTestResult> {
    ...
    console.log(
      `✅ 数据库迁移验收测试完成 (${duration}ms, ${assertions.filter(a => a.passed).length}/${assertions.length} passed)`
    );
    ...
    return {
      ...
      artifacts,
      coverage: {
        repositoryContracts: 0.7,
        databaseTables: 1.0,
        migrationScripts: 1.0,
        dataModels: 0.8,
      },
    };
  }
  ...
  private async validateUserVersion(
  ...
  /** 验证用户数据目录路径 */
  private async validateUserDataPath(
    assertions: TestAssertion[]
  ): Promise<void> {
    ...
    try {
      ...
      assertions.push({
        ...
        passed:
          actualUserDataPath ===
          this.electronController.testEnvironment.userDataPath,
...
abstract class RepositoryContractAcceptanceTest {
  ...
  private async testBasicCrudOperations(): Promise<void> {
    ...
    this.testResults.push({
      ...
      artifacts: [],
      coverage: {
        repositoryContracts: 0.8,
        databaseTables: 0.6,
        migrationScripts: 0.5,
        dataModels: 0.7,
      },
    });

    console.log(
      `  ✅ CRUD操作测试完成 (${duration}ms, ${assertions.filter(a => a.passed).length}/${assertions.length} passed)`
    );
  }
}
```
...
class DataIntegritySmokeTest {
  ...
  async executeDataIntegritySmokeTests(): Promise<AcceptanceTestResult> {
    ...
    console.log(
      `✅ 数据完整性烟雾测试完成 (${duration}ms, ${assertions.filter(a => a.passed).length}/${assertions.length} passed)`
    );
    ...
    return {
      ...
      artifacts,
      coverage: {
        repositoryContracts: 1.0,
        databaseTables: 1.0,
        migrationScripts: 0.8,
        dataModels: 1.0,
      },
    };
  }
  ...
  /** 测试完整的角色生命周期 */
  private async testCharacterLifecycle(
    assertions: TestAssertion[]
  ): Promise<void> {
...
class InPlaceAcceptanceTestRunner {
  ...
  async runAllTests(): Promise<AcceptanceTestSuite> {
    ...
    try {
      ...
      // 2. 执行数据库迁移验收测试
      console.log('\n📊 执行数据库迁移验收测试...');
      const migrationValidator = new DatabaseMigrationValidator(
        this.electronController
      );
      const migrationResult =
        await migrationValidator.validateDatabaseMigrations();
      this.testResults.push(migrationResult);

      // 3. 执行Repository契约验收测试
      console.log('\n🔗 执行Repository契约验收测试...');
      const contractTest = new RepositoryContractAcceptanceTest(
        this.electronController
      );
