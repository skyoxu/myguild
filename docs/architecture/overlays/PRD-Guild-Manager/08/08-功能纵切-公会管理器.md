...
class Guild implements AggregateRoot {
  ...
  recruitMember(
    ...
    // 发布成员招募事件
    this.publishEvent(new MemberRecruitedEvent(
      this.id,
      member.id,
  ...
  organizeRaid(
    ...
    // 发布副本开始事件
    this.publishEvent(new RaidStartedEvent(
      this.id,
      raidInstance.id,
      raidConfig.raidType,
...
class GuildMember implements Entity {
  ...
  updateSatisfaction(change: number, reason: SatisfactionChangeReason): void {
    ...
    if (oldLevel !== this.satisfaction.level) {
      DomainEvents.raise(new MemberSatisfactionChangedEvent(
        this.id,
        oldLevel,
  ...
  gainExperience(
    ...
    DomainEvents.raise(new MemberExperienceGainedEvent(
      this.id,
      activityType,
      experienceGained,
...
class CompositionValidationService implements DomainService {
  ...
  async validateComposition(
    ...
    // 验证角色配置
    const roleValidation = this.validateRoleDistribution(composition, raidConfig);
    errors.push(...roleValidation);
    
    // 验证成员可用性
    const availabilityValidation = await this.validateMemberAvailability(composition);
  ...
  private validateRoleDistribution(
    ...
    if (tankCount < requirements.minTanks || tankCount > requirements.maxTanks) {
      errors.push(new ValidationError(
        "INVALID_TANK_COUNT",
    ...
    if (healerCount < requirements.minHealers || healerCount > requirements.maxHealers) {
      errors.push(new ValidationError(
...
class CombatCalculationService implements DomainService {
  ...
  async calculateRaidResult(
    ...
    // 应用战术加成
    const tacticModifiers = this.calculateTacticModifiers(selectedTactics, raidConfig);
    ...
    const casualties = this.calculateCasualties(composition, isSuccess, difficultyModifier);
...
class SQLiteMemberRepository implements MemberRepository {
  ...
  async updateSatisfaction(memberId: string, satisfaction: number): Promise<void> {
    const sql = `
      UPDATE members 
  ...
  describe('副本组织', () => {
    it('应该成功组织副本活动', async () => {
      ...
      await guild.recruitMultipleMembers([
        new MemberCandidate("Tank1", CharacterClass.WARRIOR, "TANK", 60, RecruitmentSource.SEARCH),
        new MemberCandidate("Healer1", CharacterClass.PRIEST, "HEALER", 55, RecruitmentSource.SEARCH),
        new MemberCandidate("DPS1", CharacterClass.MAGE, "DPS", 58, RecruitmentSource.SEARCH),
        new MemberCandidate("DPS2", CharacterClass.ROGUE, "DPS", 57, RecruitmentSource.SEARCH),
        new MemberCandidate("DPS3", CharacterClass.HUNTER, "DPS", 59, RecruitmentSource.SEARCH)
      ], new ResourceCost({ gold: 500 }));
      
      const raidConfig = RaidConfigurationFactory.createSmallDungeon();
      const composition = RaidCompositionBuilder
        .create("Test Raid", raidConfig.raidType)
        .addTank(guild.getMember("Tank1"))
        .addHealer(guild.getMember("Healer1"))
        .addDPS([guild.getMember("DPS1"), guild.getMember("DPS2"), guild.getMember("DPS3")])
        .build();
...
describe('CompositionValidationService', () => {
  ...
  it('应该验证阵容角色分配', async () => {
    ...
    const composition = RaidCompositionBuilder
      .create("Invalid Composition", RaidType.MEDIUM_DUNGEON)
    ...
    // Act
    const result = await validationService.validateComposition(composition, raidConfig);
...
describe('公会管理器完整流程', () => {
  ...
  test('完整的公会管理流程', async () => {
    ...
    // 验证公会创建成功
    await expect(page.locator('[data-testid="guild-name-display"]')).toHaveText('E2E测试公会');
    ...
    await page.click('[data-testid="member-candidate-0"] [data-testid="recruit-btn"]');
    ...
    // 拖拽成员到坦克槽位
    const memberCard = page.locator('[data-testid="available-members"] [data-testid="member-0"]');
    const tankSlot = page.locator('[data-testid="tank-slots"] [data-testid="slot-0"]');
    ...
    // 选择战术
    await page.click('[data-testid="tactic-selection"] [data-testid="tactic-0"]');
    
    // 开始副本
    await page.click('[data-testid="start-raid-btn"]');
    
    // 等待战斗结果
    await page.waitForSelector('[data-testid="raid-result"]', { timeout: 10000 });
    ...
    // 5. 检查成员经验增长
    await page.click('[data-testid="member-management-tab"]');
    const memberExp = await page.textContent('[data-testid="member-0-experience"]');
  ...
  test('亲密度系统交互', async () => {
    ...
    // 验证初始联系人数量
    const initialContacts = await page.locator('[data-testid="contact-list"] [data-testid="contact-item"]').count();
    
    // 执行社交行动
    if (initialContacts > 0) {
      await page.click('[data-testid="contact-item-0"] [data-testid="action-menu"]');
      ...
      // 验证亲密度变化
      const intimacyAfter = await page.textContent('[data-testid="contact-item-0"] [data-testid="intimacy-value"]');
  ...
  test('性能监控验证', async () => {
    ...
    await page.click('[data-testid="member-management-tab"]');
    await page.waitForSelector('[data-testid="member-table"]', { state: 'visible' });
