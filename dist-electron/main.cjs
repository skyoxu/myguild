...
if (process.env.SECURITY_TEST_MODE === 'true' || process.env.E2E_AUTO_START === '1') {
    ...
    electron_1.app.commandLine.appendSwitch('disable-renderer-backgrounding');
    console.log('📋 [main.ts] Minimal test switches applied - removed potentially problematic options');
}
...
function createSecureBrowserWindow() {
    ...
    win.webContents.setWindowOpenHandler(({ url }) => {
        ...
        const isTrusted = trustedDomains.some(domain => urlObj.hostname === domain || urlObj.hostname.endsWith(`.${domain}`));
        if (isTrusted &&
            (url.startsWith('https://') || url.startsWith('http://'))) {
            console.log(`🌐 [setWindowOpenHandler] 通过shell.openExternal打开受信任链接: ${url}`);
            electron_1.shell.openExternal(url);
        }
        else {
    ...
    win.webContents.on('did-fail-load', (_, errorCode, errorDescription, validatedURL, isMainFrame) => {
        if (isMainFrame && errorCode !== 0) {
            console.log(`🔄 [did-fail-load] 主框架加载失败 (${errorCode}): ${errorDescription}, URL: ${validatedURL}`);
            // 检查是否是chrome-error页面，立即阻止
            if (validatedURL && validatedURL.startsWith('chrome-error://')) {
                console.log(`🚫 [did-fail-load] 检测到chrome-error页面，立即重定向到安全页面`);
                // 立即加载安全的本地页面
...
function createWindow(is, ses) {
    ...
    mainWindow.webContents.on('did-fail-load', (_, errorCode, errorDescription, validatedURL) => {
        console.log(`❌ [did-fail-load] 页面加载失败: ${errorCode} - ${errorDescription} - ${validatedURL}`);
    });
    // ✅ 修复chrome-error://问题：使用loadFile替代loadURL('app://')
    const isDev = !!process.env.VITE_DEV_SERVER_URL;
    if (isDev && process.env.VITE_DEV_SERVER_URL) {
        console.log(`📂 [loadURL] 开发环境加载: ${process.env.VITE_DEV_SERVER_URL}`);
...
electron_1.app.whenReady().then(async () => {
    ...
    ses.webRequest.onBeforeRequest({ urls: ['*://*/*'] }, (details, callback) => {
        ...
        const isSubresourceAllowed = details.url.startsWith('app://') ||
        ...
        if (!isSubresourceAllowed) {
            console.log(`🚫 [webRequest] Block subresource: ${details.url} (${details.resourceType})`);
        }
