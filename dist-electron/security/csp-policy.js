...
class CSPManager {
     ...
     * 生成开发环境CSP（通过响应头注入）
     */
    static generateDevelopmentCSP(nonce) {
    ...
    static validateCSP(csp) {
        const missingDirectives = this.REQUIRED_DIRECTIVES.filter(directive => !csp.includes(directive));
        const risks = [];
        // 检查危险值
     ...
     * 检查两个CSP策略的兼容性
     */
    static checkPolicyCompatibility(policy1, policy2) {
        const conflicts = [];
        const suggestions = [];
        // 解析策略差异
