...
class SecurityPolicyManager {
    ...
    constructor(isProduction = process.env.NODE_ENV === 'production') {
        ...
        console.log(`🔒 初始化安全策略管理器 (${isProduction ? '生产' : '开发'}环境)`);
    }
    /**
     * 记录安全审计日志
     */
    ...
    getSecurityAuditReport() {
        ...
        const securityScore = totalEvents > 0 ? Math.round((deniedEvents / totalEvents) * 100) : 100;
        return {
            totalEvents,
            allowedEvents,
            deniedEvents,
    ...
    setupPermissionHandler(ses) {
        ses.setPermissionRequestHandler((_webContents, permission, callback, details) => {
            ...
            // 检查源是否被允许
            const isOriginAllowed = this.config.allowedOrigins.some(origin => requestingOrigin.startsWith(origin));
            // 检查权限是否被允许
            const isPermissionAllowed = this.config.allowedPermissions.includes(permission);
            // 对于敏感权限，即使配置允许也要额外检查
            const sensitivePermissions = ['media', 'geolocation', 'notifications'];
            if (sensitivePermissions.includes(permission)) {
                // 生产环境默认拒绝敏感权限
                if (this.isProduction) {
                    this.logSecurityEvent('permission', 'deny', `生产环境拒绝敏感权限: ${permission} from ${requestingOrigin}`);
                    callback(false);
                    return;
                }
            }
            ...
            // 记录安全审计日志
            this.logSecurityEvent('permission', shouldAllow ? 'allow' : 'deny', `${permission} from ${requestingOrigin} (源允许: ${isOriginAllowed}, 权限允许: ${isPermissionAllowed})`);
    ...
    setupNavigationHandler(window) {
        window.webContents.on('will-navigate', (event, navigationUrl) => {
            ...
            // 检查是否为允许的本地导航
            const isLocalNavigation = this.config.allowedOrigins.some(origin => targetOrigin.startsWith(origin));
            // 检查是否为允许的域名导航
            const isDomainAllowed = this.config.allowedNavigationDomains.some(domain => targetHostname === domain || targetHostname.endsWith('.' + domain));
            if (!isLocalNavigation && !isDomainAllowed) {
                this.logSecurityEvent('navigation', 'deny', `阻止导航到: ${navigationUrl} (hostname: ${targetHostname})`);
                event.preventDefault();
                // 可选：显示用户友好的错误消息
                // dialog.showErrorBox('导航被阻止', `不允许导航到: ${targetHostname}`);
            }
            else {
                this.logSecurityEvent('navigation', 'allow', `允许导航到: ${navigationUrl} (local: ${isLocalNavigation}, domain: ${isDomainAllowed})`);
            }
        });
        // 防止加载外部内容到webview
        window.webContents.on('will-attach-webview', (event, _webPreferences, params) => {
            const targetOrigin = new URL(params.src).origin;
            const isOriginAllowed = this.config.allowedOrigins.some(origin => targetOrigin.startsWith(origin));
            if (!isOriginAllowed) {
                this.logSecurityEvent('navigation', 'deny', `阻止webview加载: ${params.src} (origin: ${targetOrigin})`);
                event.preventDefault();
            }
            else {
                this.logSecurityEvent('navigation', 'allow', `允许webview加载: ${params.src}`);
            }
        });
    }
    /**
    ...
    setupWindowOpenHandler(window) {
        window.webContents.setWindowOpenHandler(({ url }) => {
            ...
            // 检查是否为允许的外部域名
            const isExternalAllowed = this.config.allowedExternalDomains.some(domain => targetHostname === domain || targetHostname.endsWith('.' + domain));
            if (isExternalAllowed) {
                this.logSecurityEvent('window-open', 'allow', `在外部浏览器打开: ${url} (hostname: ${targetHostname})`);
                electron_1.shell.openExternal(url);
            }
            else {
                this.logSecurityEvent('window-open', 'deny', `阻止打开外部链接: ${url} (hostname: ${targetHostname})`);
            }
            // 总是拒绝在新窗口中打开
            return { action: 'deny' };
        });
    ...
    setupWebRequestFiltering(ses) {
        ses.webRequest.onBeforeRequest((details, callback) => {
            ...
            // 检查是否为允许的外部请求
            const isOriginAllowed = this.config.allowedOrigins.some(origin => details.url.startsWith(origin));
            if (!isOriginAllowed && this.isProduction) {
                this.logSecurityEvent('web-request', 'deny', `阻止外部请求: ${details.url} (protocol: ${url.protocol})`);
                callback({ cancel: true });
            }
            else {
                if (!isOriginAllowed) {
                    // 开发环境允许但记录日志
                    this.logSecurityEvent('web-request', 'allow', `开发环境允许外部请求: ${details.url}`);
                }
                callback({ cancel: false });
            }
        });
