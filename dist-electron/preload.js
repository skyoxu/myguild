...
if (process.contextIsolated) {
    try {
        ...
        electron_1.contextBridge.exposeInMainWorld('electronAPI', Object.freeze({
            ...
            bringToFront: () => {
                if (process.env.CI === 'true' || process.env.NODE_ENV === 'test') {
                    // 通过IPC请求主进程前置窗口
                    preload_1.electronAPI.ipcRenderer?.invoke?.('window:bring-to-front');
                }
            },
        ...
        if (process.env.SECURITY_TEST_MODE === 'true') {
            electron_1.contextBridge.exposeInMainWorld('__SECURITY_VALIDATION__', Object.freeze({
                isSandboxed: process.sandboxed,
                contextIsolated: process.contextIsolated,
            ...
            electron_1.contextBridge.exposeInMainWorld('__PARAM_VALIDATION__', Object.freeze({
        ...
        electron_1.contextBridge.exposeInMainWorld('__APP_VERSION__', process.env.APP_VERSION || '0.1.1');
        // 简化的测试API标识（减少信息泄露）
        electron_1.contextBridge.exposeInMainWorld('__CUSTOM_API__', Object.freeze({
            preloadExposed: true,
        }));
    }
    catch (error) {
