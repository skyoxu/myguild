...
import { getWebVitalsMonitor, getWebVitalsCollector, WebVitalsMetrics } from '../shared/observability/web-vitals-monitor';
...
export function useWebVitals(options: UseWebVitalsOptions = {}): WebVitalsHookReturn {
  ...
  const collector = useRef(enabled ? getWebVitalsCollector(collectorConfig) : null);
  ...
  useEffect(() => {
    ...
    return () => {
      if (renderStartTime.current > 0) {
        ...
        monitor.current?.measure(`${componentName}_render`, `${componentName}_render_start`, `${componentName}_render_end`);
        
        // 记录渲染时间
        if (renderDuration > 50) { // 超过50ms的渲染才记录
          console.log(`[WebVitals] 组件"${componentName}"渲染耗时: ${renderDuration.toFixed(2)}ms`);
        }
      }
    };
  ...
  const endTiming = useCallback((name: string) => {
    ...
    console.log(`[WebVitals] ${componentName} - ${name}: ${duration.toFixed(2)}ms`);
    return duration;
  }, [enabled, componentName]);

  // 记录交互事件
  const recordInteraction = useCallback((name: string, duration: number) => {
    if (!enabled || !monitor.current) return;
    monitor.current.recordInteraction(`${componentName}_${name}`, duration);
  }, [enabled, componentName]);

  // 记录事件处理
  const recordEvent = useCallback((name: string, duration: number) => {
    if (!enabled || !monitor.current) return;
...
export function usePerformanceTracker(name: string, enabled = true) {
  ...
  const measure = useCallback(async <T extends any>(fn: () => T | Promise<T>): Promise<T> => {
    start();
    try {
...
export function useRoutePerformance(enabled = true) {
  ...
  const startRouteChange = useCallback((fromRoute: string, toRoute: string) => {
    if (!enabled || !monitor.current) return;
...
export function useDataFetchPerformance(enabled = true) {
  ...
  const trackFetch = useCallback(async <T extends any>(
    endpoint: string,
    fetchFn: () => Promise<T>
  ): Promise<T> => {
    ...
    try {
      ...
      monitor.current.measure(`fetch_${endpoint}`, `fetch_${endpoint}_start`, `fetch_${endpoint}_end`);
