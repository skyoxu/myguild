...
export async function setupMainProcessMonitoring() {
  const { initializeMainProcessMonitoring } = await import('./metrics-integration');
...
export async function setupRendererProcessMonitoring() {
  const { initializeRendererProcessMonitoring } = await import('./metrics-integration');
...
export class BattleManager {
  ...
  async executeBattleRound(playerCount: number = 2) {
    ...
    try {
      ...
      recordBattleRoundTime(roundTime, this.battleType, this.currentRound, playerCount);
...
export class UIPerformanceMonitor {
  static monitorComponentRender<T>(
    ...
    try {
      ...
      recordUIRenderTime(renderTime, componentName, renderTime > 100 ? 'complex' : 'simple');
      
      if (renderTime > 100) {
        console.warn(`⚠️ 组件${componentName}渲染耗时${renderTime}ms，可能需要优化`);
      }
...
export function GameLevelComponent({ levelId }: { levelId: string }) {
  ...
  const loadLevel = async () => {
    setLoading(true);
    const measureDelay = UIPerformanceMonitor.measureInteractionDelay('level-load', 'GameLevelComponent');
  ...
  return UIPerformanceMonitor.monitorComponentRender('GameLevelComponent', () => (
...
export async function validateMonitoringIntegration(): Promise<boolean> {
  ...
  try {
    ...
    console.log(`${allHealthy ? '✅' : '⚠️'} 监控系统${allHealthy ? '运行正常' : '部分异常'}`);
