...
function checkBundleSize() {
  ...
  for (const [filePath, limit] of Object.entries(SIZE_LIMITS)) {
    ...
    if (result.violation) {
      ...
      console.log(`✅ ${filePath}`)
      console.log(`   大小: ${formatBytes(actualSize)} (${percentage}% of limit)`)
    }
    console.log()
  }
  ...
  console.log(`总大小: ${formatBytes(totalActual)} / ${formatBytes(totalLimit)} (${overallPercentage}%)`)
  console.log(`违规文件: ${results.filter(r => r.violation).length} / ${results.length}`)
  
  if (hasViolations) {
    console.log('❌ Bundle size check failed!')
    console.log('💡 建议: 检查是否引入了不必要的大型依赖，考虑代码分割或懒加载')
    process.exit(1)
  } else {
