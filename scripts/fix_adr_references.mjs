...
class Logger {
  constructor() {
    this.logFile = path.join(LOGS_DIR, `adr-fix-${new Date().toISOString().split('T')[0]}.log`);
  }
...
class AdrReferencesFixer {
  ...
  async processAllFiles() {
    ...
    await this.logger.info('开始批量处理PRD文件', { totalFiles: prdFiles.length });
  ...
  async generateReport() {
    ...
    const reportPath = path.join(LOGS_DIR, `adr-fix-report-${new Date().toISOString().split('T')[0]}.json`);
...
// 如果直接运行此文件
if (process.argv[1] && process.argv[1].replace(/\\/g, '/').includes('fix_adr_references.mjs')) {
  main();
}
