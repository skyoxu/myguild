...
async function walk(dir) {
  const out = [];
  for (const ent of await fs.readdir(dir, { withFileTypes: true })) { // fs.Dirent
...
const rel = p => p.split(path.sep).join('/').replace(REPO.replace(/\\/g,'/') + '/', '');
...
async function buildBase() {
  const files = (await walk(BASE_DIR))
    ...
    .map(rel)
    .sort(naturalSort);
  await fs.writeFile('architecture_base.index', files.join('\n') + '\n', 'utf8');
