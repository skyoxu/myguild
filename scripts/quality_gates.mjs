...
async function checkReleaseHealthGate() {
  ...
  if (!fs.existsSync(releaseHealthPath)) {
    console.log('⚠️  未找到 .release-health.json 文件，跳过 Release Health 检查');
    return {
      passed: true,
      skipped: true,
  ...
  console.log(`  Crash-Free Sessions: ${releaseHealth.crashFreeSessions || 'N/A'}%`);
  ...
  if (releaseHealth.crashFreeSessions !== undefined) {
    if (releaseHealth.crashFreeSessions < HARD_CODED_THRESHOLDS.releaseHealth.crashFreeSessions) {
      failedChecks.push(`Crash-Free Sessions ${releaseHealth.crashFreeSessions}% < ${HARD_CODED_THRESHOLDS.releaseHealth.crashFreeSessions}%`);
    }
  }
  
  if (releaseHealth.crashFreeUsers !== undefined) {
    if (releaseHealth.crashFreeUsers < HARD_CODED_THRESHOLDS.releaseHealth.crashFreeUsers) {
      failedChecks.push(`Crash-Free Users ${releaseHealth.crashFreeUsers}% < ${HARD_CODED_THRESHOLDS.releaseHealth.crashFreeUsers}%`);
    }
  }
  
  if (releaseHealth.adoption !== undefined) {
    if (releaseHealth.adoption < HARD_CODED_THRESHOLDS.releaseHealth.minAdoption) {
      failedChecks.push(`Adoption ${releaseHealth.adoption} < ${HARD_CODED_THRESHOLDS.releaseHealth.minAdoption}`);
    }
  }
  
  if (failedChecks.length > 0) {
    throw new Error(`Release Health 门禁失败:\\n${failedChecks.map(check => `  - ${check}`).join('\\n')}`);
  }
...
async function checkSentryHealth() {
  ...
  // 检查 Sentry 配置文件
  const sentryFiles = [
    path.join(__dirname, '..', 'src', 'shared', 'observability', 'sentry-main.ts'),
    path.join(__dirname, '..', 'src', 'shared', 'observability', 'sentry-renderer.ts')
  ];
