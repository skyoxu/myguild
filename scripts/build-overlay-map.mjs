...
const mdFiles = await fg([`${OVERLAYS.replace(/\\/g,'/')}/**/08/**/*.md`], { dot:true });
...
const stop = new Set([
  // 英文通用词
  'the','a','an','of','to','for','and','with','by','on','in','at','from',
  // 中文通用词
  '功能','模块','管理','管理器','系统','开发','实现','构建',
  // 文件名/数字类（低信号）
  '08','overlays','md','task','tasks','acceptance','checklist',
  // 通用技术词（容易误命中）
  'member','成员','data','数据','report','报告','api','interface'
]);

function tokens(s=''){
  return String(s).replace(/[^\p{L}\p{N}\s-]/gu,' ')
...
function addKw(k, p){
  ...
  // 使用相对POSIX路径
  const relativePath = path.relative(process.cwd(), path.resolve(p)).replace(/\\/g, '/');
...
for (const file of mdFiles){
  ...
  const kw = new Set([
    ...tokens(title),
    ...tokens([...(fm.keywords||[]), ...(fm.tags||[]), ...(fm.aliases||[])].join(' ')),
  ...
  if (title.toLowerCase().includes('guild')) { addKw('guild', rel); addKw('公会', rel); }
  ...
  // 查找验收清单
  let accept = '';
  for (const cand of ['ACCEPTANCE_CHECKLIST.md','Acceptance.md','ACCEPTANCE.md']){
    try { 
      await fs.access(path.join(path.dirname(abs), cand)); 
      accept = path.relative(process.cwd(), path.join(path.dirname(abs), cand)).replace(/\\/g, '/'); 
      break; 
    } catch {}
  }
  ...
  const anchorPatterns = [
    ...
    { key: 'persistence', pattern: /##\s*8\.5\s*持久化层设计|##\s*持久化层?设计/m },
...
console.log(`✅ overlay-map written: ${OUT} (files=${Object.keys(files).length}, keywords=${Object.keys(keywords).length})`);