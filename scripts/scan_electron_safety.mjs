...
function analyzeBrowserWindowConfig(content, filePath) {
  ...
  for (const match of matches) {
    ...
    // 检查 nodeIntegration
    if (!config.includes('nodeIntegration') || config.includes('nodeIntegration: true')) {
      issues.push({
        type: 'security',
        severity: 'critical',
        file: filePath,
    ...
    // 检查 contextIsolation
    if (!config.includes('contextIsolation') || config.includes('contextIsolation: false')) {
      issues.push({
        type: 'security',
        severity: 'critical',
        file: filePath,
  ...
  for (const api of nodeApis) {
    if (content.includes(api) && !content.includes(`// ${api} is safe`)) {
      issues.push({
        ...
        recommendation: '避免在预加载脚本中直接使用 Node.js API，使用 contextBridge 白名单机制'
      });
    }
  }
  ...
  for (const pattern of unsafePatterns) {
    if (content.includes(pattern)) {
      issues.push({
        ...
        recommendation: '避免使用 eval、Function 构造函数等不安全的代码执行方式'
      });
    }
  }
...
function analyzeCSP(content, filePath) {
  ...
  // 检查是否有 CSP meta 标签
  const cspMetaPattern = /<meta[^>]*http-equiv=['"](Content-Security-Policy|content-security-policy)['"]/i;
  ...
  if (!cspMatch) {
    issues.push({
      ...
      recommendation: '添加严格的 CSP meta 标签，例如：<meta http-equiv="Content-Security-Policy" content="default-src \'self\'; script-src \'self\'; style-src \'self\' \'unsafe-inline\';">'
    });
    return issues;
  }
  ...
  const unsafePatterns = [
    { pattern: 'unsafe-inline', severity: 'medium', message: '使用了 unsafe-inline，存在 XSS 风险' },
    { pattern: 'unsafe-eval', severity: 'high', message: '使用了 unsafe-eval，存在代码注入风险' },
    { pattern: /\*(?!\.[a-zA-Z])/, severity: 'medium', message: '使用了通配符 *，CSP 过于宽松' },
    { pattern: 'data:', severity: 'low', message: '允许 data: URL，可能存在安全风险' }
  ];
  
  for (const { pattern, severity, message } of unsafePatterns) {
    if (typeof pattern === 'string' ? cspContent.includes(pattern) : pattern.test(cspContent)) {
      issues.push({
        type: 'security',
        severity,
        file: filePath,
        message: `CSP 配置不安全: ${message}`,
        recommendation: '使用更严格的 CSP 配置，避免 unsafe-inline 和 unsafe-eval'
      });
    }
  }
...
function generateSecurityReport(allIssues) {
  ...
  const reportFile = path.join(reportDir, `electron-security-scan-${timestamp}.json`);
...
function generateRecommendations(issues) {
  ...
  if (issues.some(i => i.message.includes('nodeIntegration'))) {
    recommendations.push('在所有 BrowserWindow 配置中设置 nodeIntegration: false');
  }
  
  if (issues.some(i => i.message.includes('contextIsolation'))) {
    recommendations.push('在所有 BrowserWindow 配置中设置 contextIsolation: true');
  }
  ...
  if (issues.some(i => i.message.includes('CSP'))) {
    recommendations.push('在所有 HTML 文件中添加严格的 Content-Security-Policy meta 标签');
  }
  
  if (issues.some(i => i.message.includes('contextBridge'))) {
    recommendations.push('在预加载脚本中使用 contextBridge.exposeInMainWorld 暴露白名单 API');
  }
