...
async function callSentryAPI(endpoint, options = {}) {
  ...
  try {
    log('debug', 'Calling Sentry API', { url, method: options.method || 'GET' })
...
async function getReleaseSessionStats(version, timeWindow = '24h') {
  ...
  const params = new URLSearchParams({
    ...
    field: ['sum(session)', 'count_unique(user)', 'p95(session.duration)', 'crash_rate_sessions', 'crash_rate_users'],
...
function analyzeHealthData(healthStats, sessionStats, errorStats, version) {
  ...
  try {
      ...
      sessionStats.groups.forEach(group => {
        if (group.by && group.by.release === version) {
          ...
          if (crashRateSessions.length > 0) {
            const latestSessionRate = crashRateSessions[crashRateSessions.length - 1][1]
            analysis.metrics.crashFreeSessions = Math.max(0, 100 - (latestSessionRate * 100))
          }
          
          if (crashRateUsers.length > 0) {
            const latestUserRate = crashRateUsers[crashRateUsers.length - 1][1]
            analysis.metrics.crashFreeUsers = Math.max(0, 100 - (latestUserRate * 100))
          }
        }
      })
    }
    ...
    if (errorStats && errorStats.data) {
      const totalEvents = errorStats.data.reduce((sum, point) => sum + (point[1] || 0), 0)
    ...
    if (analysis.metrics.crashFreeSessions !== null) {
      if (analysis.metrics.crashFreeSessions < thresholds.crashFreeSessions.critical) {
      ...
      } else if (analysis.metrics.crashFreeSessions < thresholds.crashFreeSessions.warning) {
    ...
    if (analysis.metrics.crashFreeUsers !== null) {
      if (analysis.metrics.crashFreeUsers < thresholds.crashFreeUsers.critical) {
        analysis.healthy = false
        analysis.issues.push({
          type: 'critical',
      ...
      } else if (analysis.metrics.crashFreeUsers < thresholds.crashFreeUsers.warning) {
        analysis.warnings.push({
          type: 'warning',
          metric: 'crashFreeUsers',
          value: analysis.metrics.crashFreeUsers,
    ...
    if (analysis.issues.length > 0) {
      analysis.recommendations.push({
        ...
        message: 'Critical health metrics detected - immediate rollback recommended'
      })
    } else if (analysis.warnings.length > 0) {
      analysis.recommendations.push({
        priority: 'high',
        action: 'investigate',
        message: 'Health metrics below warning thresholds - investigation recommended'
      })
    }
...
async function triggerRollback(version, reason) {
  ...
  const rollbackScript = path.join(__dirname, 'release', 'execute-rollback.mjs')
  ...
  if (fs.existsSync(rollbackScript)) {
    try {
      ...
      return new Promise((resolve, reject) => {
        const child = spawn('node', [rollbackScript, '--emergency', `--reason=${reason}`], {
...
async function main() {
  ...
  const options = {
    ...
    timeWindow: process.argv.find(arg => arg.startsWith('--time-window='))?.split('=')[1] || HEALTH_CONFIG.timeWindow.default,
  ...
  try {
    ...
    switch (command) {
      case 'check':
      case undefined:
        log('info', 'Checking release health', { version, timeWindow: options.timeWindow })
        ...
        const analysis = analyzeHealthData(healthStats, sessionStats, errorStats, version)
        
        // 保存报告
        fs.writeFileSync(HEALTH_CONFIG.output.reportPath, JSON.stringify(analysis, null, 2))
        
        // 输出结果
        console.log('\n🏥 Release Health Check Results')
        console.log('================================')
        console.log(`Version: ${analysis.version}`)
        console.log(`Health Status: ${analysis.healthy ? '✅ HEALTHY' : '❌ UNHEALTHY'}`)
        
        if (analysis.metrics.crashFreeSessions !== null) {
          console.log(`Crash-Free Sessions: ${analysis.metrics.crashFreeSessions.toFixed(2)}%`)
        }
        
        if (analysis.metrics.crashFreeUsers !== null) {
          console.log(`Crash-Free Users: ${analysis.metrics.crashFreeUsers.toFixed(2)}%`)
        }
        ...
        if (!analysis.healthy && options.autoRollback && !options.dryRun) {
          ...
          if (criticalIssues.length > 0) {
            ...
            const rollbackReason = criticalIssues.map(i => i.message).join('; ')
            const rollbackResult = await triggerRollback(version, rollbackReason)
        ...
        if (analysis.issues.length > 0 || analysis.warnings.length > 0) {
          ...
          fs.writeFileSync(HEALTH_CONFIG.output.alertPath, JSON.stringify(alerts, null, 2))
        }
