...
async function debugMain() {
  try {
    ...
    const dirExists = await fs.access(PRD_CHUNKS_DIR).then(() => true).catch(() => false);
    ...
    const prdFiles = files.filter(file => file.startsWith('PRD-Guild-Manager_chunk_') && file.endsWith('.md'));
    console.log('找到PRD文件数量:', prdFiles.length);
    console.log('文件列表:', prdFiles.slice(0, 5).map(f => f));
    
    console.log('\n3. 测试读取单个文件...');
    const testFile = path.join(PRD_CHUNKS_DIR, 'PRD-Guild-Manager_chunk_012.md');
    ...
    if (testFileExists) {
      ...
      if (endIndex > 0) {
        ...
        // 查找ADRs行
        const adrsLine = yamlLines.find(line => line.trim().startsWith('ADRs:'));
