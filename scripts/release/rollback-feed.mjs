...
export function rollbackFeed(feedFile, manifestFile, prevVersion) {
  ...
  if (!feedFile || !manifestFile || !prevVersion) {
    throw new Error('Missing required parameters: feedFile, manifestFile, and prevVersion are required');
  }
  
  if (!isValidVersion(prevVersion)) {
    throw new Error(`Invalid version format: ${prevVersion}. Expected semantic version (e.g., 1.2.3)`);
  }
  
  if (!isValidFeedFile(feedFile)) {
    throw new Error(`Invalid feed file: ${feedFile}. Expected latest.yml, latest-mac.yml, or latest-linux.yml`);
  }
  ...
  if (!prevVersionData) {
    ...
    throw new Error(`Version ${prevVersion} not found in manifest. Available versions: ${availableVersions}`);
  }
  ...
  if (missingFields.length > 0) {
    throw new Error(`Version ${prevVersion} missing required fields: ${missingFields.join(', ')}`);
  }
...
if (isMainModule) {
  ...
  if (!feedFile || !manifestFile || !prevVersion) {
    ...
    console.error('Usage: node scripts/release/rollback-feed.mjs <feedFile> <manifestFile> <prevVersion>');
  ...
  try {
    ...
    console.error(`📦 Rolled back from ${result.previousVersion} to ${result.rolledBackTo}`);
