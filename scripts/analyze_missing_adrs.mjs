...
async function analyzeSpecificFiles() {
  ...
  for (const fileName of testFiles) {
    ...
    for (const [adrId, analysis] of Object.entries(scores)) {
      ...
      console.log(`  ${status} ${adrId}: ${analysis.score.toFixed(3)} (阈值: ${analysis.threshold})`);
      
      // 显示匹配的关键词
      if (analysis.matchedKeywords.length > 0) {
        console.log(`    匹配关键词: ${analysis.matchedKeywords.map(k => `${k.keyword}(${k.frequency})`).join(', ')}`);
      }
      
      if (!analysis.shouldInclude && analysis.score > 0.25) {
        console.log(`    ⚠️  得分${analysis.score.toFixed(3)}接近阈值，可能需要降低阈值`);
      }
    }
  ...
  const chunk020Content = await fs.readFile(path.join(PRD_CHUNKS_DIR, 'PRD-Guild-Manager_chunk_020.md'), 'utf8');
  
  // 检查contracts路径出现
  const contractMatches = chunk020Content.match(/src\/shared\/contracts/g) || [];
  console.log(`chunk_020中"src/shared/contracts"出现: ${contractMatches.length} 次`);
  
  // 检查数据架构关键内容
  const hasDataArchitecture = chunk020Content.includes('数据管理规格') || 
                             chunk020Content.includes('数据架构') ||
                             chunk020Content.includes('数据统计模块');
  console.log(`chunk_020包含数据架构内容: ${hasDataArchitecture}`);
  
  // 检查UI相关内容
  const chunk012Content = await fs.readFile(path.join(PRD_CHUNKS_DIR, 'PRD-Guild-Manager_chunk_012.md'), 'utf8');
  const hasUIContent = chunk012Content.includes('用户界面规格') ||
  ...
  const improvedRules = {
    'ADR-0007': {
      ...
      newKeywords: [
        ...
        '数据架构', '数据管理', '统计模块',  // 中文关键词
        'types', 'events', 'interfaces'  // TypeScript相关
      ]
    },
    'ADR-0009': {
      threshold: 0.25,  // 降低阈值
      newKeywords: [
        '用户界面', '界面设计', '交互', '响应式',  // 中文UI关键词
        'modal', 'navigation', 'layout', 'responsive'  // 更多UI关键词
      ]
    }
  };
