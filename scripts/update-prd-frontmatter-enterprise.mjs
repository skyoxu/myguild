...
const writeFileUtf8 = (filePath, content) => fs.writeFileSync(filePath, content, { encoding: 'utf8' });

// PRD分片分类定义
const PRD_CLASSIFICATION = {
  overview: ['chunk_001', 'chunk_024'],
  technical: ['chunk_002', 'chunk_003', 'chunk_004', 'chunk_005', 'chunk_006', 'chunk_010', 'chunk_020', 'chunk_021'],  
  functional: ['chunk_007', 'chunk_008', 'chunk_009', 'chunk_011', 'chunk_012', 'chunk_013', 'chunk_014', 'chunk_015', 'chunk_016', 'chunk_017', 'chunk_018', 'chunk_019', 'chunk_022', 'chunk_023']
};
...
function generateEnterpriseFrontMatter(chunkNumber, classification, fileSize) {
  const baseConfig = {
    ...
    'Test-Refs': [`tests/unit/guild-manager-chunk-${chunkNumber.toString().padStart(3, '0')}.spec.ts`],
    'Monitors': [`txn.prd-guild-manager_chunk_${chunkNumber.toString().padStart(3, '0')}.primary`],
  ...
  const contractDefinitions = generateContractDefinitions(chunkNumber, classification);
...
function generateReleaseGates(classification) {
  const baseGates = {
    ...
    Security_Gate: {
      ...
      blockingFailures: ["security_vulnerabilities", "dependency_vulnerabilities"],
      windowHours: 12
    },
    Performance_Gate: {
      enabled: true,
...
function generateContractDefinitions(chunkNumber, classification) {
  const baseContract = {
    types: [`src/shared/contracts/guild/chunk-${chunkNumber.toString().padStart(3, '0')}-types.ts`],
    events: {
      specversion: "1.0",
  ...
  // 技术接口型分片增强契约定义
  if (classification === 'technical') {
    baseContract.interfaces = [`src/shared/contracts/guild/chunk-${chunkNumber.toString().padStart(3, '0')}-interfaces.ts`];
    baseContract.validation_rules = [`src/shared/validation/chunk-${chunkNumber.toString().padStart(3, '0')}-validation.ts`];
  }
...
function updatePRDChunk(filePath) {
  ...
  try {
    ...
    console.log(`  - Chunk ${chunkNumber}, 分类: ${classification}, 大小: ${fileSize} chars`);
    
    // 生成新的front-matter
    const frontMatter = generateEnterpriseFrontMatter(chunkNumber, classification, fileSize);
...
function main() {
  ...
  // 获取所有PRD chunk文件
  const files = fs.readdirSync(prdChunksDir)
