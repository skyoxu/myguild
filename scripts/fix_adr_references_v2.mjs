...
const ADR_MAPPING_RULES = {
  'ADR-0007': {
    ...
    keywords: [
      // 原有关键词
      'contract', 'interface', 'repository', 'storage', 'API',
      'validation_rules', 'Contract_Definitions', 'dataschema',
      'ports', 'adapters', 'abstraction', 'persistence',
      // 新增关键词
      'src/shared/contracts', '数据架构', '数据管理', '统计模块',
      'types', 'events', 'interfaces', 'CrossReferenceEngine',
  ...
  'ADR-0008': {
    name: '部署与发布策略',
    keywords: [
      'Release_Gates', 'monitoring', 'SLO', 'deployment', 'performance',
      'Quality_Gate', 'Security_Gate', 'Performance_Gate', 'Acceptance_Gate',
      'windowHours', 'threshold', 'blockingFailures'
    ],
    weight: 0.5,
  ...
  'ADR-0009': {
    ...
    keywords: [
      // 原有关键词
      'user-experience', 'UI', 'ActionType', 'component', 'interaction',
      'interface', 'navigation', 'usability', 'responsive', 'accessibility',
      'cross-platform', 'adaptation',
      // 新增关键词
      '用户界面', '界面设计', '交互', '响应式', '用户体验',
      'modal', 'layout', 'UISpecifications', 'NavigationBar',
      'tooltip', 'keyboard', 'screen'
    ],
    weight: 0.6,
...
class Logger {
  constructor() {
    this.logFile = path.join(LOGS_DIR, `adr-fix-v2-${new Date().toISOString().split('T')[0]}.log`);
  }
  ...
  parse(content) {
    ...
    while (i < yamlLines.length) {
      ...
      if (colonIndex > 0) {
        ...
        const isListField = ['ADRs', 'Arch-Refs', 'Test-Refs', 'Monitors', 'SLO-Refs', 'Depends-On'].includes(key);
          ...
          if (value.startsWith('[') && value.endsWith(']')) {
            ...
            while (i < yamlLines.length) {
              ...
              if (nextTrimmed.startsWith('- ')) {
                const itemValue = nextTrimmed.substring(2).trim().replace(/^["']|["']$/g, '');
  ...
  updateAdrs(yamlLines, newAdrs) {
    ...
    if (!adrsUpdated) {
      ...
      for (let j = 0; j < updatedLines.length; j++) {
        ...
        if (trimmed.startsWith('Priority:') || trimmed.startsWith('Risk:') || 
          ...
          if (trimmed.startsWith('Depends-On:')) {
            ...
            while (k < updatedLines.length) {
              ...
              if (nextTrimmed.startsWith('- ') || nextTrimmed === '' || nextTrimmed.startsWith('#')) {
                k++;
              } else {
                break;
              }
...
class AdrReferencesFixerV2 {
  ...
  async getPrdFiles() {
    try {
      ...
      await this.logger.info('发现PRD文件', { count: prdFiles.length, files: prdFiles });
  ...
  async processPrdFile(filePath) {
    try {
      ...
      await this.logger.error(`处理文件失败: ${path.basename(filePath)}`, { error: error.message });
    }
  }
  ...
  async processAllFiles() {
    ...
    await this.logger.info('开始批量处理PRD文件', { totalFiles: prdFiles.length, version: 'v2.0' });
  ...
  async generateReport() {
    ...
    const reportPath = path.join(LOGS_DIR, `adr-fix-v2-report-${new Date().toISOString().split('T')[0]}.json`);
    ...
    await this.logger.info('处理完成，生成v2.0报告', {
      ...
      summary: {
        ...
        totalAdrsAdded: this.processedFiles.reduce((sum, file) => sum + file.addedAdrs.length, 0)
      }
    });
    ...
    console.log(`新增ADR引用总数: ${this.processedFiles.reduce((sum, file) => sum + file.addedAdrs.length, 0)}`);
...
async function main() {
  ...
  try {
    ...
    await fixer.logger?.error('脚本执行失败', { error: error.message, stack: error.stack });
    process.exit(1);
  }
}

if (process.argv[1] && process.argv[1].replace(/\\/g, '/').includes('fix_adr_references_v2.mjs')) {
  main();
}
