...
function loadWebVitalsData() {
  try {
    ...
    const mockDataPath = path.join(__dirname, '..', 'logs', 'mock-web-vitals-data.json');
      ...
      data.push({
        ...
        metrics: {
          lcp: {
            ...
            rating: baseLCP * regressionFactor <= 2500 ? 'good' : baseLCP * regressionFactor <= 4000 ? 'needs-improvement' : 'poor'
          },
          inp: {
            id: `inp-${timestamp}`,
            name: 'INP', 
            value: baseINP * regressionFactor,
            rating: baseINP * regressionFactor <= 200 ? 'good' : baseINP * regressionFactor <= 500 ? 'needs-improvement' : 'poor'
          },
          cls: {
            id: `cls-${timestamp}`,
            name: 'CLS',
            value: baseCLS * regressionFactor,
            rating: baseCLS * regressionFactor <= 0.1 ? 'good' : baseCLS * regressionFactor <= 0.25 ? 'needs-improvement' : 'poor'
          },
          fcp: {
            id: `fcp-${timestamp}`,
            name: 'FCP',
            value: baseFCP * regressionFactor,
            rating: baseFCP * regressionFactor <= 1800 ? 'good' : baseFCP * regressionFactor <= 3000 ? 'needs-improvement' : 'poor'
          },
          ttfb: {
            id: `ttfb-${timestamp}`,
            name: 'TTFB',
            value: baseTTFB * regressionFactor,
            rating: baseTTFB * regressionFactor <= 800 ? 'good' : baseTTFB * regressionFactor <= 1800 ? 'needs-improvement' : 'poor'
          },
          customTimings: {
...
function calculateBaseline(data) {
  const cutoff = Date.now() - (GATE_CONFIG.baseline.windowDays * 24 * 60 * 60 * 1000);
  const baselineData = data.filter(point => point.timestamp > cutoff);
  
  if (baselineData.length < GATE_CONFIG.baseline.minSampleSize) {
    throw new Error(`基线数据不足: ${baselineData.length} < ${GATE_CONFIG.baseline.minSampleSize}`);
  }
  
  // 提取各指标数值
  const lcpValues = baselineData.filter(d => d.metrics.lcp).map(d => d.metrics.lcp.value);
  const inpValues = baselineData.filter(d => d.metrics.inp).map(d => d.metrics.inp.value);
  const clsValues = baselineData.filter(d => d.metrics.cls).map(d => d.metrics.cls.value);
  const fcpValues = baselineData.filter(d => d.metrics.fcp).map(d => d.metrics.fcp.value);
  const ttfbValues = baselineData.filter(d => d.metrics.ttfb).map(d => d.metrics.ttfb.value);
...
function performGateCheck(baseline, recent) {
  ...
  metrics.forEach(metric => {
    if (baseline.metrics[metric].count === 0 || recent.metrics[metric].count === 0) {
    ...
    const regressionPercent = ((recentValue - baselineValue) / baselineValue) * 100;
...
function generateTrendData(data) {
  ...
  metrics.forEach(metric => {
    ...
    if (trends[metric].daily.length >= 7) {
      ...
      const firstWeekAvg = firstWeek.reduce((sum, day) => sum + day.p95, 0) / firstWeek.length;
      const lastWeekAvg = lastWeek.reduce((sum, day) => sum + day.p95, 0) / lastWeek.length;
...
`;
  ...
  Object.entries(trends).forEach(([metric, data]) => {
    const trendIcon = data.overall.trend === 'improving' ? '📈' : 
                     data.overall.trend === 'degrading' ? '📉' : '➡️';
...
function saveReports(report, markdown, trends) {
  try {
    ...
    // 保存JSON报告
    fs.writeFileSync(GATE_CONFIG.output.reportPath, JSON.stringify(report, null, 2));
    
    // 保存Markdown摘要
    fs.writeFileSync(GATE_CONFIG.output.summaryPath, markdown);
    
    // 保存趋势数据
    fs.writeFileSync(GATE_CONFIG.output.trendPath, JSON.stringify(trends, null, 2));
    ...
    switch (command) {
      ...
      case undefined:
        ...
        if (gateResults.failures.length > 0) {
          ...
          gateResults.failures.forEach(failure => {
            console.log(`  - ${failure.metric}: P95回归${failure.regressionPercent.toFixed(1)}%`);
          });
        }
      ...
      case 'baseline':
        ...
        ['LCP', 'INP', 'CLS', 'FCP', 'TTFB'].forEach(metric => {
          ...
          if (m.count > 0) {
            console.log(`${metric}: P50=${m.p50.toFixed(2)}, P95=${m.p95.toFixed(2)} (${m.count}个样本)`);
          }
        });
      ...
      case 'trends':
        ...
        Object.entries(trendsOnly).forEach(([metric, data]) => {
          console.log(`${metric}: ${data.overall.trend} (${data.overall.changePercent > 0 ? '+' : ''}${data.overall.changePercent.toFixed(1)}%)`);
        });
