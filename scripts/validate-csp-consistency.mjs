...
class CSPValidator {
  ...
  validatePolicy(policyName, csp, environment = 'production') {
    ...
    if (missingDirectives.length > 0) {
      this.errors.push(`${policyName}: 缺少必需的CSP指令: ${missingDirectives.join(', ')}`);
      isValid = false;
    }

    // 检查安全约束
    for (const [directive, expectedValues] of Object.entries(SECURITY_REQUIREMENTS.securityConstraints)) {
      if (directives[directive]) {
        ...
        if (!hasAllRequired) {
          const missing = expectedValues.filter(value => 
            !directives[directive].includes(value)
          );
          this.errors.push(`${policyName}: ${directive}指令缺少安全值: ${missing.join(', ')}`);
          isValid = false;
        }
      }
    }
    ...
    for (const [directive, values] of Object.entries(directives)) {
      ...
      if (foundForbidden.length > 0) {
        this.errors.push(`${policyName}: ${directive}包含禁止的危险值: ${foundForbidden.join(', ')}`);
        isValid = false;
      }
    }
    ...
    if (environment === 'production') {
      if (directives['script-src'] && directives['script-src'].includes("'unsafe-inline'")) {
        this.errors.push(`${policyName}: 生产环境禁止在script-src中使用'unsafe-inline'`);
        isValid = false;
      }
    }
  ...
  extractIndexCSP() {
    ...
    const cspMatch = content.match(/http-equiv="Content-Security-Policy"[^>]*content="([^"]+)"/);
  ...
  validateCSPManagerExists() {
    const cspManagerPath = join(ROOT_DIR, 'electron', 'security', 'csp-policy.ts');
    
    if (!existsSync(cspManagerPath)) {
      this.errors.push('CSPManager模块文件不存在: electron/security/csp-policy.ts');
      return false;
    }
  ...
  async validate() {
    ...
    this.log(`\n验证完成: ${allValid ? '通过' : '失败'}`, allValid ? 'info' : 'error');
