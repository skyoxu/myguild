...
if (!existsSync(resultsPath)) {
  console.error('[e2e-summary] test-results/test-results.json not found. Run npm run test:e2e first.');
  process.exit(2);
}
...
function classify(item) {
  ...
  const inSecurity = file.includes('\\security\\') || file.includes('/security/') || proj.includes('security');
  if (inSecurity) return { level: 'P0', category: 'Electron安全基线/导航/权限' };

  const inSmoke = file.includes('\\smoke\\') || file.includes('/smoke/') || file.endsWith('perf.smoke.spec.ts');
  if (inSmoke) {
    const isBoot = /(render|启动|可见|protocol|窗口|launch|visible)/.test(title);
    return isBoot
      ? { level: 'P0', category: '启动与基本渲染' }
      : { level: 'P1', category: '交互P95/轻性能' };
  }

  if (file.includes('scene-transition')) return { level: 'P1', category: '场景转换性能' };
  if (file.includes('framerate')) return { level: 'P1', category: '帧率稳定性' };
  if (file.includes('\\quality\\') || file.includes('/quality/')) return { level: 'P3', category: '质量护栏/框架一致性' };
  if (file.includes('guild-manager') || file.includes('vertical-slice') || file.includes('playable')) return { level: 'P3', category: '领域/可玩度验证' };
...
const failures = all.filter(x => x.status && x.status !== 'passed' && x.status !== 'skipped');
...
writeFileSync(
  ...
  JSON.stringify({
    summary: { passed: passed.length, skipped: skipped.length, failures: failures.length },
    buckets,
  }, null, 2),
  'utf8'
);
...
txt.push(`总失败: ${failures.length}  |  通过: ${passed.length}  |  跳过: ${skipped.length}`);
txt.push('');
...
writeFileSync(resolve(logsDir, `e2e-px-summary-${stamp}.txt`), txt.join('\n'), 'utf8');

console.log(`[e2e-summary] 输出: logs/e2e-px-summary-${stamp}.json | .txt`);
console.log('[e2e-summary] 提示：P0=安全/启动，P1=可观测/轻性能，P2=构建/一致性(非E2E)，P3=领域无关/可玩度。');
