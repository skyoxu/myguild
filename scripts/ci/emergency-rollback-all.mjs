...
for (const feed of feeds) {
  if (fs.existsSync(feed)) {
    ...
    results.push({ feed, success: r.status === 0, result: (r.stdout || '').trim() });
  } else {
    results.push({ feed, success: true, skipped: true });
  }
}
const out = JSON.stringify(results);
if (process.env.GITHUB_OUTPUT) fs.appendFileSync(process.env.GITHUB_OUTPUT, `rollback_results=${out}\n`);
