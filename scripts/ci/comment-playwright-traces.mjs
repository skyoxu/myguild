...
const head = manifest.entries.slice(0, 10).map((e, i) => `- ${i+1}. ${rel(e.target)} (${Math.round((e.size||0)/1024)} kB)`).join('\n');
...
async function createComment(issue_number) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`Failed to post comment: ${r.status} ${r.statusText} - ${t}`);
  }
}
