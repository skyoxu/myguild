...
if (status === 'passed' && stage !== '100') {
  ...
  lines.push(`- Continue to ${next}% stage`, '- Wait 10-15 minutes before next stage');
} else if (status === 'failed') {
  lines.push('- Health check failed - immediate action required', '- Review release metrics and error reports');
} else if (stage === '100' && status === 'passed') {
  lines.push('- Gradual release completed. Application is fully deployed.', '- Consider updating PREV_GA_VERSION to current version');
}
