...
function setOut(k, v){ if(out) fs.appendFileSync(out, `${k}=${v}\n`, 'utf8'); }

const r = spawnSync(process.execPath, ['scripts/soft-gate-reporter.js'], { stdio: 'inherit', shell: process.platform === 'win32' });
if (r.status !== 0) {
  ...
  setOut('soft-gate-summary', 'Issues encountered during execution, please check logs');
  process.exit(0);
}
