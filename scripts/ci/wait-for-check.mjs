...
const optional = String(process.env.OPTIONAL || '').toLowerCase() === 'true' || String(process.env.SKIP_IF_NOT_FOUND || '').toLowerCase() === 'true';
...
const logFile = path.join(dateDir, `gatekeeper-wait-check-${hh}${mi}${ss}Z.log`);
try { fs.mkdirSync(dateDir, { recursive: true }); } catch {}
const log = (msg) => {
...
if (targetCandidates.length === 0) {
  throw new Error('At least one of TARGET_CHECK_NAME or TARGET_JOB_NAME is required');
}
...
function matchCheckRunName(name) {
  ...
  for (const t of targetCandidates) {
    ...
    if (t.includes(' / ')) {
      ...
      if (jobOnly && (name === jobOnly || name.startsWith(jobOnly))) return true;
    }
  }
  return false;
}
...
async function main(){
  ...
  log(`candidates=${JSON.stringify(targetCandidates)} workflowHint=${targetWorkflowName || '-'} optional=${optional}`);

  while (Date.now() - start < timeoutMs) {
    const checks = await listChecks();

    if (!printedCatalogue) {
      const cat = checks.map(c => `${c.name} [status=${c.status} conclusion=${c.conclusion}]`).join(', ');
    ...
    log(`Waiting for check (match by job name): found=${!!cr} status=${status} conclusion=${conclusion}`);
    ...
    if (!cr) {
      ...
      if (optional && elapsed >= notFoundGraceMs) {
        log(`Target check not found after ${Math.round(elapsed/1000)}s, OPTIONAL=true → skip waiting.`);
        return;
      }
    }
...
main().catch(err => { log(String(err)); process.exit(1); });
