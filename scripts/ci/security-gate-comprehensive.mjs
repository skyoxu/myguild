...
class SecurityGate {
  ...
  async validateE2ESecurityTests() {
    ...
    try {
      ...
      if (passedChecks < securityChecks.length) {
        this.log(`E2E安全检查不完整: ${passedChecks}/${securityChecks.length}`, 'error');
        return false;
      }
  ...
  async validateCloudEventsContracts() {
    ...
    try {
      // 运行CloudEvents单元测试
      const result = await this.runCommand('npm run test:unit -- tests/unit/contracts/cloudevents.spec.ts');
      ...
      // 检查源代码安全实现
      const cloudEventsFile = join(ROOT_DIR, 'src/shared/contracts/cloudevents-core.ts');
      ...
      const missingChecks = securityChecks.filter(check => !content.includes(check));
      if (missingChecks.length > 0) {
        this.log(`CloudEvents缺少安全检查: ${missingChecks.join(', ')}`, 'error');
        return false;
      }
      ...
      const foundDangerous = dangerousPatterns.filter(pattern => content.includes(pattern));
      if (foundDangerous.length > 0) {
        this.log(`CloudEvents包含危险模式: ${foundDangerous.join(', ')}`, 'error');
        return false;
      }
    ...
    try {
      ...
      for (const [setting, expectedValue] of Object.entries(securitySettings)) {
        ...
        if (!pattern.test(mainContent)) {
          this.log(`Electron安全设置错误: ${setting} 应为 ${expectedValue}`, 'error');
          return false;
        }
      }
      ...
      const foundDangerous = dangerousConfigs.filter(config => mainContent.includes(config));
      if (foundDangerous.length > 0) {
  ...
  async runDependencySecurityScan() {
    ...
    try {
      // npm audit检查
      const auditResult = await this.runCommand('npm audit --json', { stdio: 'pipe' });
      ...
      if (auditResult.output) {
        ...
        for (const [severity, count] of Object.entries(counts)) {
          ...
          if (count > maxAllowed) {
            this.log(`${severity.toUpperCase()}级别漏洞超标: ${count}/${maxAllowed}`, 'error');
        ...
        this.log(`依赖安全扫描: Critical:${counts.critical}, High:${counts.high}, Moderate:${counts.moderate}, Low:${counts.low}`, 'info');
      }
  ...
  comparePolicies(policy1, policy2) {
    ...
    const similarity = common / Math.max(normalized1.length, normalized2.length);
  ...
  async run() {
    ...
    this.log(`通过检查: ${this.results.filter(r => r.passed).length}/${this.results.length}`);
