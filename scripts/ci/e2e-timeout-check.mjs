...
try {
  if (!raw.trim()) {
    console.log('[config-check] E2E_SECURITY_TIMEOUT_MS not set, skipping validation');
    process.exit(0);
  }
  const val = Number(raw);
  if (!Number.isFinite(val)) {
    console.log(`[config-check] E2E_SECURITY_TIMEOUT_MS is not a valid number: ${raw}, using default`);
    process.exit(0);
  }
  if (val < 60000 || val > 900000) {
    console.log(`[config-check] E2E_SECURITY_TIMEOUT_MS out of range [60000,900000]: ${val}, using default`);
    process.exit(0);
  }
