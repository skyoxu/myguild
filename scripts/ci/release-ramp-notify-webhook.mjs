...
if (rollback) {
  status = 'Rollback Executed'; color = 'danger'; message = `Release ramp failed health check at ${stage}% and was automatically rolled back`;
} else if (healthPassed) {
  status = `Stage ${stage}% Complete`; color = 'good'; message = `Release ramp to ${stage}% completed successfully with healthy metrics`;
} else {
  status = `Stage ${stage}% - No Health Check`; color = 'warning'; message = `Release ramp to ${stage}% completed (health check skipped)`;
}
...
const body = {
  ...
  attachments: [{
    color,
    fields: [
      { title: 'App Version', value: process.env.APP_VERSION || '', short: true },
