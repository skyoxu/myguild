    ...
    if (errorIssues.length === 0) {
      ...
      for (const issue of errorIssues) {
        ...
        if (issue.type === 'missing-job') {
          console.log(
            `    💡 修复建议：在 ${issue.file} 中添加缺失的job '${issue.missingDep}'，或从 '${issue.job}' 的needs中移除`
          );
        }
      }
    }
    ...
    if (warningIssues.length > 0) {
      ...
      for (const issue of warningIssues) {
        ...
        if (issue.type === 'core-gate-conditional') {
          console.log(
            `    💡 建议：将路径过滤移至workflow触发条件而非job级if条件`
          );
        }
      }
    }
    ...
    if (process.env.GITHUB_OUTPUT) {
      import('fs').then(fs => {
        fs.appendFileSync(
          ...
          `workflow_check_errors=${errorIssues.length}\\n`
        );
        fs.appendFileSync(
          process.env.GITHUB_OUTPUT,
          `workflow_check_warnings=${warningIssues.length}\\n`
        );
      });
    }
...
if (
  ...
  process.argv[1].includes('workflow-consistency-check.mjs')
) {
  checkWorkflows();
}
