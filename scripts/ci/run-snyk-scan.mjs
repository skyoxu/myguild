...
const install = spawnSync('npm', ['install', '-g', 'snyk'], { stdio: 'inherit', shell: process.platform === 'win32' });
if (install.status !== 0) {
  console.warn('Failed to install snyk (non-blocking).');
  process.exit(0);
}
const test = spawnSync('snyk', ['test', '--severity-threshold=high', '--json'], { stdio: 'inherit', shell: process.platform === 'win32' });
if (test.status !== 0) {
