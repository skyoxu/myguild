...
const ANALYSIS_CONFIG = {
  ...
  // 重点关注的依赖
  watchDependencies: [
    'react', 'react-dom', 'antd', 'phaser', '@ant-design/icons',
    '@sentry/browser', '@sentry/electron'
  ],
...
function generateRecommendations(bundleAnalysis, dependencyAnalysis) {
  ...
  const largeJsFiles = jsFiles.filter(f => f.size > ANALYSIS_CONFIG.thresholds.largeDependency)
...
function generateReport(bundleAnalysis, dependencyAnalysis, recommendations) {
  const report = {
    ...
    summary: {
      ...
      totalSizeFormatted: bundleAnalysis ? formatBytes(bundleAnalysis.totalSize) : '0 Bytes',
  ...
  try {
    ...
    switch (command) {
      ...
      case undefined:
        ...
        const recommendations = generateRecommendations(bundleAnalysis, dependencyAnalysis)
        
        log('info', 'Creating report...')
        const report = generateReport(bundleAnalysis, dependencyAnalysis, recommendations)
        
        // 保存JSON报告
        fs.writeFileSync(ANALYSIS_CONFIG.output.reportPath, JSON.stringify(report, null, 2))
