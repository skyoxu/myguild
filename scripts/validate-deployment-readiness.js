...
class DeploymentValidator {
    ...
    for (const script of requiredScripts) {
      ...
      try {
        ...
        try {
          ...
          this.addCheck('基础设施', script.description, 'pass', '脚本存在且语法正确');
        } catch (syntaxError) {
          this.addCheck('基础设施', script.description, 'fail', 
            '脚本语法错误', syntaxError.message);
        }
    ...
    for (const workflow of requiredWorkflows) {
      ...
      try {
        ...
        if (content.includes('name:') && content.includes('on:') && content.includes('jobs:')) {
          // 检查是否包含统一门禁相关的脚本调用
          const hasCorrectScripts = this.validateWorkflowScripts(content, workflow.path);
          
          if (hasCorrectScripts) {
            this.addCheck('工作流', workflow.description, 'pass', 'YAML结构正确且包含必需脚本');
          } else {
            this.addCheck('工作流', workflow.description, 'warn', 'YAML结构正确但脚本调用可能需要检查');
          }
        } else {
          this.addCheck('工作流', workflow.description, 'fail', 'YAML结构不完整');
        }
        
      } catch (error) {
        this.addCheck('工作流', workflow.description, 'fail', 
          '工作流文件不存在或无法读取', workflow.path);
      }
    }
  }
  ...
  validateWorkflowScripts(content, workflowPath) {
    const scriptMappings = {
      'security-unified.yml': ['guard:security', 'scripts/security-gate-wrapper.js'],
      'observability-gate.yml': ['guard:observability', 'scripts/observability-config-validation.js'],
  ...
  async validatePackageScripts() {
    ...
    try {
      ...
      if (missingScripts.length === 0) {
        this.addCheck('配置', 'package.json脚本', 'pass', '所有必需的npm脚本都已配置');
      } else {
        this.addCheck('配置', 'package.json脚本', 'fail', 
          '缺少必需的npm脚本', `缺少: ${missingScripts.join(', ')}`);
      }
      
    } catch (error) {
      this.addCheck('配置', 'package.json脚本', 'fail', 
        '无法读取或解析package.json', error.message);
    }
  }
    ...
    for (const test of functionalTests) {
      try {
        ...
        if (test.name.includes('软门禁')) {
          this.addCheck('功能测试', test.name, 'warn', '软门禁执行异常（正常情况）', 
            '软门禁设计为不阻塞流程');
        } else {
          this.addCheck('功能测试', test.name, 'fail', '执行失败', 
    ...
    for (const doc of requiredDocs) {
      ...
      try {
        ...
        if (hasTitle && hasContent) {
          ...
          this.addCheck('文档', doc.description, 'warn', '文档存在但内容可能不完整');
        }
  ...
  async generateReport() {
    ...
    // 保存详细报告
    const reportPath = path.join(this.projectRoot, 'logs', 'deployment-readiness-report.json');
