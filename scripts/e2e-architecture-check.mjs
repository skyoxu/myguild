...
class E2EArchitectureChecker {
   ...
   */
  async check() {
    console.log(`${colors.cyan}${colors.bold}=== E2E 测试架构标准检查 ===${colors.reset}\n`);
    ...
    console.log(`${colors.blue}找到 ${testFiles.length} 个测试文件${colors.reset}\n`);
  ...
  checkImportConsistency(filePath, content) {
    ...
    if (wrongImports) {
      this.addError(`${relativePath}: 发现错误的 playwright 导入，应使用 @playwright/test`, {
        file: relativePath,
        issue: 'wrong_import',
    ...
    if (hasCorrectImport) {
      ...
      this.addWarning(`${relativePath}: 未发现 @playwright/test 导入，可能不是 Playwright 测试文件`);
    }
  }
  ...
  checkTestStructure(filePath, content) {
    ...
    if (hasBeforeAll && hasAfterAll) {
      ...
      this.addWarning(`${relativePath}: 未发现 beforeAll/afterAll 钩子，建议添加以管理应用生命周期`);
    } else {
      this.addWarning(`${relativePath}: 测试钩子不完整，建议同时使用 beforeAll 和 afterAll`);
    }
  ...
  checkSelectorUsage(filePath, content) {
    ...
    if (hasDataTestId && hasWaitForSelector) {
      ...
      this.addWarning(`${relativePath}: 建议使用 data-testid 属性选择器以提高测试稳定性`);
    }
    ...
    for (const pattern of fragileSelectors) {
      const matches = content.match(pattern);
      if (matches) {
        this.addWarning(`${relativePath}: 发现可能脆弱的选择器，建议使用更稳定的选择器`, {
  ...
  generateReport() {
    ...
    console.log(`${colors.green}✓ 通过项: ${this.passed.length}${colors.reset}`);
    console.log(`${colors.yellow}⚠ 警告项: ${this.warnings.length}${colors.reset}`);
...
async function main() {
  try {
    ...
    console.error(`${colors.red}${colors.bold}检查过程中发生错误:${colors.reset}`, error);
    process.exit(1);
  }
}
