...
const CONFIG_LINKAGE_RULES = {
  ...
  'tech-versions': {
    ...
    configs: ['react-version', 'typescript-version', 'electron-version', 'tailwind-version'],
  ...
  'security-policies': {
    ...
    configs: ['csp-policy', 'node-integration', 'context-isolation', 'sandbox-mode'],
...
async function validateConfigConsistency(adrList) {
  ...
  for (const ruleName of Object.keys(CONFIG_LINKAGE_RULES)) {
    ...
    for (const targetId of rule.targets) {
      ...
      // 验证配置同步
      const configIssues = await validateRuleCompliance(sourceAdr, targetAdr, rule);
    ...
    switch (rule.syncRule) {
      ...
      case 'inherit-strict':
        ...
        for (const config of rule.configs) {
          ...
          if (sourcePolicy && !isStrictInheritance(sourcePolicy, targetPolicy, config)) {
            issues.push({
      ...
      case 'min-threshold':
        ...
        for (const config of rule.configs) {
          ...
          if (sourceThreshold && targetThreshold && targetThreshold < sourceThreshold) {
            issues.push({
...
function isStrictInheritance(sourceValue, targetValue, configName) {
  ...
  switch (configName) {
    case 'csp-policy':
      // CSP策略应该包含源策略的所有限制
      return targetValue.includes('default-src \'self\'') && 
...
function generateImpactAnalysis(adr, graph, adrList) {
  ...
  return {
    ...
    riskLevel: calculateRiskLevel(directDependents.length, allAffected.size, adr.impactScope)
  };
}
...
function generateDependencyGraphDOT(adrList, graph) {
  ...
  // 添加节点
  adrList.forEach(adr => {
    const color = adr.status === 'Accepted' ? 'lightgreen' : 
                  adr.status === 'Deprecated' ? 'lightgray' : 'lightblue';
  ...
  try {
    ...
    if (configIssues.length > 0) {
      ...
      configIssues.forEach((issue, index) => {
        console.log(`   ${index + 1}. [${issue.type}] ${issue.source || 'N/A'} -> ${issue.target || issue.missing || 'N/A'}`);
        if (issue.config) {
          console.log(`      配置项: ${issue.config}`);
          if (issue.sourceValue && issue.targetValue) {
            console.log(`      源值: ${issue.sourceValue}, 目标值: ${issue.targetValue}`);
          }
        }
      });
      console.log();
      ...
      if (fixes.length > 0) {
        ...
        fixes.forEach((fix, index) => {
          console.log(`   ${index + 1}. ${fix.type}: ${fix.config} (${fix.from} -> ${fix.to})`);
        });
      } else {
    ...
    const impactAnalysis = adrList.map(adr => generateImpactAnalysis(adr, graph, adrList));
    
    // 显示高风险ADR
    const highRiskADRs = impactAnalysis.filter(analysis => analysis.riskLevel === 'HIGH');
    if (highRiskADRs.length > 0) {
      console.log(`🚨 高风险ADR (${highRiskADRs.length}个):`);
      highRiskADRs.forEach(analysis => {
        console.log(`   ${analysis.adr}: 直接影响${analysis.directDependents.length}个, 总影响${analysis.totalAffected.length}个ADR`);
      });
      console.log();
    }
    ...
    console.log('   可使用 Graphviz 渲染: dot -Tpng adr-dependencies.dot -o dependencies.png\n');
