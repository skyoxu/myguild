...
const ADR_MAPPING_RULES = {
  'ADR-0007': {
    name: '端口-适配器模式',
    keywords: ['contract', 'interface', 'repository', 'storage', 'API', 'validation_rules', 'ports', 'adapters', 'src/shared/contracts'],
    weight: 0.6,
    threshold: 0.4,
  ...
  'ADR-0008': {
    name: '部署与发布策略', 
    keywords: ['Release_Gates', 'monitoring', 'SLO', 'deployment', 'performance', 'Quality_Gate', 'Security_Gate', 'Performance_Gate'],
    weight: 0.5,
    threshold: 0.4,
  ...
  'ADR-0009': {
    name: '跨平台适配策略',
    keywords: ['user-experience', 'UI', 'ActionType', 'component', 'interaction', 'interface', 'navigation', 'UX'],
    weight: 0.6,
    threshold: 0.4,
  ...
  static parseSimpleYAML(lines) {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('- ')) {
        if (inArray) {
          arrayItems.push(trimmed.substring(2).trim().replace(/^"(.*)"$/, '$1'));
        }
        continue;
      }
  ...
  static analyzeContent(content) {
    ...
    for (const [adrId, rule] of Object.entries(ADR_MAPPING_RULES)) {
      ...
      for (const keyword of rule.keywords) {
        ...
        const regex = new RegExp(`\\b${keywordLower.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'gi');
      ...
      // 归一化得分
      const normalizedScore = rule.keywords.length > 0 ? score / rule.keywords.length : 0;
  ...
  static async fixPRDReferences(filePath) {
    try {
      ...
      if (additionalADRs.length === 0) {
        console.log(`✅ No ADR references need to be added to ${frontMatter['PRD-ID']}`);
      ...
      console.log(`📋 Total ADRs now: ${updatedADRs.length} (${updatedADRs.join(', ')})`);
...
export { ADRReferenceFixer, ContentAnalysisEngine, SafeYAMLParser, ADR_MAPPING_RULES };