...
function fixErasableSyntaxOnly(content) {
  ...
  // 修复enum导出语法
  content = content.replace(/^export const enum\s+(\w+)/gm, (match, enumName) => {
    fixes.push({
      type: 'FIX_CONST_ENUM',
      original: match,
  ...
  // 修复declare导出
  content = content.replace(/^export declare\s+(const|let|var)\s+/gm, (match, keyword) => {
    fixes.push({
...
function fixImportTypeUsage(content) {
  ...
  for (const func of functionsAsTypeImport) {
    const typeImportRegex = new RegExp(`import type \\{([^}]*${func}[^}]*)\\}`, 'g');
...
function fixEntityConstraints(content) {
  ...
  entityMissingFields.forEach(typeName => {
    ...
    if (match && !match[1].includes('createdAt') && !match[1].includes('updatedAt')) {
      const fields = match[1];
      const newFields = fields.trim() + `\n  readonly createdAt: Date;\n  readonly updatedAt: Date;\n`;
      
      content = content.replace(match[0], `export interface ${typeName} {\n${newFields}}`);
      fixes.push({
  ...
  try {
    ...
    if (modifiedContent !== content) {
      ...
      allFixes.forEach((fix, index) => {
        console.log(`   ${index + 1}. [${fix.type}] ${JSON.stringify(fix).substring(0, 100)}...`);
      });
    } else {
      console.log(`✨ 无需修复`);
    }
...
// 只有直接运行此脚本时才执行主函数
if (process.argv[1] && process.argv[1].endsWith('fix-typescript-conflicts.mjs')) {
