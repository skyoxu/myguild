...
const SCHEDULER_CONFIG = {
  ...
  checkpoint: {
    ...
    // 写入事务数阈值
    transactionThreshold: dbConfig?.checkpoint?.transactionThreshold?.threshold || 1000
  },
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stats.mtime.getTime() < cutoffTime) {
        ...
        cleaned++
        log('debug', 'Cleaned old backup', { file, age: Date.now() - stats.mtime.getTime() })
      }
    }
    
    log('info', 'Backup cleanup completed', { cleaned, retention: retentionDays })
    return { cleaned }
  } catch (error) {
...
async function performHealthCheck(dbPath) {
  try {
    ...
    try {
      ...
      const diskUsage = execSync('dir /-c', { encoding: 'utf8', cwd: path.dirname(dbPath) })
    ...
    // 5. 计算健康分数
    results.healthScore = Math.max(0, 100 - (results.issues.length * 30) - (results.warnings.length * 10))
    results.status = results.healthScore >= 80 ? 'healthy' : 
                     results.healthScore >= 60 ? 'warning' : 'critical'
...
function initializeScheduler() {
  ...
  if (SCHEDULER_CONFIG.checkpoint.intervalMinutes > 0) {
    ...
    const checkpointTimer = setInterval(async () => {
      try {
        await performSmartCheckpoint(SCHEDULER_CONFIG.dbPath, { scheduled: true })
      } catch (error) {
    ...
    log('info', 'Scheduled checkpoint timer initialized', { intervalMinutes: SCHEDULER_CONFIG.checkpoint.intervalMinutes })
  }
  ...
  if (SCHEDULER_CONFIG.health.intervalHours > 0) {
    ...
    log('info', 'Health check timer initialized', { intervalHours: SCHEDULER_CONFIG.health.intervalHours })
  }
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case 'start':
        log('info', 'Starting database scheduler', { config: SCHEDULER_CONFIG })
