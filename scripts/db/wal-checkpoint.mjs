...
async function detectSQLiteDriver() {
  const drivers = [
    ...
    { name: 'node-sqlite3-wasm', module: '@sqlite.org/sqlite-wasm', sync: true }
  ];
  ...
  throw new Error('No SQLite driver available. Please install: better-sqlite3, sqlite3, or @sqlite.org/sqlite-wasm');
}
...
function generateReport(config, analysis, checkpointResult) {
  ...
  if (analysis.wal.exists && report.postCheckpoint.wal.exists) {
    report.walSizeReduction = analysis.wal.size - report.postCheckpoint.wal.size;
    report.walSizeReductionPercent = Math.round(
      (report.walSizeReduction / analysis.wal.size) * 10000
    ) / 100;
  }
