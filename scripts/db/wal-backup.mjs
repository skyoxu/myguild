...
async function detectSQLiteCapabilities(driver) {
  try {
    ...
    if (driver.name === 'better-sqlite3') {
      ...
      try {
        const version = db.prepare('SELECT sqlite_version()').get()['sqlite_version()'];
...
function createBackupFileName(config, capabilities) {
  ...
  const timepart = new Date().toISOString().split('T')[1].split('.')[0].replace(/:/g, '');
  
  const baseName = path.basename(config.dbPath, path.extname(config.dbPath));
  const backupType = config.backupType === 'full' ? '' : `-${config.backupType}`;
  ...
  if (driver.name === 'better-sqlite3') {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.run('PRAGMA wal_checkpoint(TRUNCATE)', (checkpointErr) => {
        if (checkpointErr) {
          log('warn', 'Checkpoint warning during backup', { error: checkpointErr.message });
        }
...
async function executeOnlineBackup(config, driver, backupPath) {
  ...
  try {
    ...
    log('warn', 'Checkpoint failed during backup', { error: checkpointError.message });
  }
...
async function compressBackup(backupPath) {
  ...
  try {
    ...
    // 获取文件大小
    const originalSize = (await fs.promises.stat(backupPath).catch(() => ({ size: 0 }))).size;
    const compressedSize = (await fs.promises.stat(compressedPath)).size;
    const compressionRatio = originalSize > 0 ? Math.round((1 - compressedSize / originalSize) * 10000) / 100 : 0;
  ...
  try {
    ...
    if (driver.name === 'better-sqlite3') {
      ...
      try {
        ...
        const isOk = integrityResults.length === 1 && integrityResults[0].integrity_check === 'ok';
...
async function cleanupOldBackups(config) {
  try {
    ...
    for (const file of files) {
      if (!file.includes(path.basename(config.dbPath, path.extname(config.dbPath)))) {
        continue; // 跳过不相关的文件
      }
      ...
      if (stats.mtime.getTime() < cutoffTime) {
        ...
        deletedFiles.push({
          file,
          age: Math.round((Date.now() - stats.mtime.getTime()) / (24 * 60 * 60 * 1000))
        });
      }
    }
...
async function needsIncrementalBackup(config) {
  try {
    ...
    const backupFiles = files
      .filter(file => file.includes(path.basename(config.dbPath, path.extname(config.dbPath))))
      .map(file => ({
        name: file,
...
async function detectSQLiteDriver() {
  ...
  throw new Error('No SQLite driver available. Please install: better-sqlite3 or sqlite3');
}
  ...
  try {
    ...
    if (config.compressBackup) {
      try {
        ...
        log('warn', 'Backup compression failed', { error: compressionError.message });
        // 继续处理，不压缩
      }
    }
    ...
    if (config.verifyIntegrity) {
      try {
        ...
        log('error', 'Backup verification failed', { error: verificationError.message });
    ...
    switch (config.backupType) {
      case 'verify':
        if (config.verifyFile) {
          ...
          throw new Error('Verify mode requires --verify-only <file> parameter');
        }
        break;
