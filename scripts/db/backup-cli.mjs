...
function checkSQLiteAvailability() {
  try {
    ...
    log('error', 'SQLite3 command line tool not found. Please install sqlite3.', {
      error: error.message
    });
    return false;
  }
...
function getDatabaseInfo(dbPath) {
  try {
    ...
    const journalMode = execSync(`sqlite3 "${dbPath}" "${walQuery}"`, { 
      encoding: 'utf8' 
    }).trim();
    
    // 获取页面计数
    const pageCountQuery = "PRAGMA page_count;";
    const pageCount = parseInt(execSync(`sqlite3 "${dbPath}" "${pageCountQuery}"`, { 
      encoding: 'utf8' 
    }).trim());
    ...
    log('error', 'Failed to get database info', { path: dbPath, error: error.message });
    throw error;
  }
}
...
async function performCheckpoint(dbPath) {
  ...
  try {
    ...
    const result = execSync(`sqlite3 "${dbPath}" "${checkpointQuery}"`, { 
      encoding: 'utf8' 
    }).trim();
    ...
    log('warn', 'WAL checkpoint failed', { path: dbPath, error: error.message });
    // 继续执行，检查点失败不应终止备份
  }
}
  ...
  try {
    ...
    const cleanup = () => {
      try {
        ...
        log('warn', 'Failed to release backup lock', { lockFile, error: error.message });
      }
    };
    ...
    log('error', 'Failed to acquire backup lock', { lockFile, error: error.message });
    throw error;
  }
}
...
async function executeVacuumIntoBackup(srcPath, destPath) {
  try {
    log('info', 'Starting VACUUM INTO backup', { src: srcPath, dest: destPath });
    const startTime = performance.now();
    
    // 使用VACUUM INTO执行备份
    const vacuumQuery = `VACUUM INTO '${destPath.replace(/'/g, "''")}'`;
    execSync(`sqlite3 "${srcPath}" "${vacuumQuery}"`, { 
      stdio: 'pipe' 
    });
...
async function executeBackupCommand(srcPath, destPath) {
  try {
    log('info', 'Starting .backup command backup', { src: srcPath, dest: destPath });
    const startTime = performance.now();
    
    // 使用.backup命令执行备份
    const backupCommand = `.backup '${destPath.replace(/'/g, "''")}'`;
    execSync(`sqlite3 "${srcPath}" "${backupCommand}"`, { 
      stdio: 'pipe' 
    });
...
async function verifyBackup(backupPath, originalPath, deep = true) {
  try {
    ...
    try {
      ...
      const integrityResult = execSync(`sqlite3 "${backupPath}" "${integrityQuery}"`, { 
        encoding: 'utf8',
        timeout: 30000 // 30秒超时
      }).trim();
      ...
      if (!integrityPassed) {
        log('error', 'Backup failed integrity check', { result: integrityResult });
      }
    } catch (error) {
      results.push({ 
        test: 'integrity_check', 
      ...
      log('warn', 'Integrity check failed to complete', { error: error.message });
    }
    
    // 深度验证：表计数对比
    if (deep) {
      try {
        const tableCountQuery = "SELECT COUNT(*) FROM sqlite_master WHERE type='table';";
        const backupTableCount = parseInt(execSync(`sqlite3 "${backupPath}" "${tableCountQuery}"`, { 
          encoding: 'utf8' 
        }).trim());
        const originalTableCount = parseInt(execSync(`sqlite3 "${originalPath}" "${tableCountQuery}"`, { 
          encoding: 'utf8' 
        }).trim());
...
async function compressBackup(backupPath) {
  ...
  try {
    ...
    log('info', 'Compressing backup', { src: backupPath, dest: compressedPath });
    ...
    log('error', 'Backup compression failed', { path: backupPath, error: error.message });
    throw error;
  }
}
...
function selectBackupMode(dbInfo) {
  ...
  if (dbInfo.isWalMode && dbInfo.size > 100 * 1024 * 1024) { // 100MB
    return 'vacuum';
  }
  
  // 如果WAL文件较大，使用backup命令更安全
  if (dbInfo.walSize > 10 * 1024 * 1024) { // 10MB
    return 'backup';
  }
...
async function performBackup() {
  ...
  try {
    ...
    log('info', 'Selected backup mode', { mode: finalMode, auto: mode === 'auto' });
...
export { performBackup, getDatabaseInfo, executeVacuumIntoBackup, executeBackupCommand, verifyBackup };