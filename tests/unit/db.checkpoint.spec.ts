...
function createTestWALDatabase(dbPath: string) {
  ...
  try {
    ...
    // 强制写入一些 WAL 数据
    db.prepare('INSERT INTO test_data (content) VALUES (?)').run('wal test data');
...
test.skipIf(!isBetterSQLite3Available())('checkpoint TRUNCATE mode clears WAL file', () => {
  // 验证 WAL 文件存在
...
test.skipIf(!isBetterSQLite3Available())('checkpoint handles non-WAL databases gracefully', () => {
  // 创建非 WAL 模式数据库
...
test('checkpoint script parameter validation', () => {
  ...
  expect(() => {
    execSync(
      'node scripts/db/checkpoint.mjs --database=./non-existent.db',
      { stdio: 'pipe' }
    );
  }).toThrow();
});
...
test('checkpoint script help output', () => {
  const helpOutput = execSync(
    'node scripts/db/checkpoint.mjs --help',
    { encoding: 'utf8', stdio: 'pipe' }
  );
...
test.skipIf(!isBetterSQLite3Available())('checkpoint integration with guard:ci', () => {
...
test.skipIf(!isBetterSQLite3Available())('checkpoint error handling and recovery', () => {
  // 测试错误处理机制
...
test('checkpoint script JSON output format consistency', () => {
  ...
  try {
    execSync(
      'node scripts/db/checkpoint.mjs --database=./invalid-path.db',
      { encoding: 'utf8', stdio: 'pipe' }
    );
  } catch (error: any) {
    if (error.stdout) {
