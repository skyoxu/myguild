...
test.skipIf(!isSQLite3Available())('backup creates a valid sqlite file', () => {
  execSync(`node scripts/db/backup-cli.mjs ${TEST_DB_PATH} ${TEST_BACKUP_DIR} --mode=backup`, { stdio: 'inherit' });
  const latest = fs.readdirSync(TEST_BACKUP_DIR).filter(f => f.endsWith('.sqlite')).sort().pop();
...
test.skipIf(!isSQLite3Available())('VACUUM INTO mode creates compressed backup', () => {
  // 测试 VACUUM INTO 模式（适合“冷备/轻写”窗口）
  execSync(`node scripts/db/backup-cli.mjs ${TEST_DB_PATH} ${TEST_BACKUP_DIR} --mode=vacuum`, { stdio: 'inherit' });
  const latest = fs.readdirSync(TEST_BACKUP_DIR).filter(f => f.endsWith('.sqlite')).sort().pop();
  expect(latest).toBeTruthy();
...
test.skipIf(!isSQLite3Available())('backup handles concurrent access gracefully', () => {
  // 模拟并发访问场景
  const promises = [];
...
test.skipIf(!isSQLite3Available())('checkpoint integration with backup workflow', () => {
  ...
  // 3. 验证备份文件生成
  const backups = fs.readdirSync(TEST_BACKUP_DIR).filter(f => f.endsWith('.sqlite'));
  ...
  expect(backupContent.subarray(0, expectedHeader.length)).toEqual(expectedHeader);
});

test.skipIf(!isSQLite3Available())('backup error handling and validation', () => {
  // 测试错误处理和验证机制
