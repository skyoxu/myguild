...
test.beforeAll(async () => {
  ...
  electronApp = await electron.launch({
    ...
    timeout: 30000,
    recordVideo: process.env.CI ? { dir: 'test-results/security-redlines' } : undefined,
  });
...
test.describe('🔴 Electron安全红线测试 - ADR-0002核心拦截', () => {
  ...
  test.describe('红线1: 权限请求拦截 (默认拒绝)', () => {
    test('地理位置权限应被默认拒绝', async () => {
      ...
      console.log(`[RedLine] ✅ 地理位置权限被拒绝: ${geolocationResult.reason}`);
    });
  ...
  test.describe('红线3: 新窗口打开拦截 (零容忍)', () => {
    test('所有window.open调用都应被阻止', async () => {
      ...
      allOpenTests.forEach(test => {
        ...
        console.log(`[RedLine] ✅ 阻止窗口打开: ${test.url} (target: ${test.target})`);
      });
      
      const totalBlocked = allOpenTests.filter(test => test.blocked).length;
      console.log(`[RedLine] ✅ 全部${totalBlocked}/${allOpenTests.length}个窗口打开被阻止`);
    });
          ...
          return new Promise(resolve => {
            iframe.onload = () => {
              try {
                ...
                if (iframeWindow) {
                  const popup = iframeWindow.open('https://malicious.com', '_blank');
                  resolve({ 
                    iframeCreated: true,
                    popupBlocked: !popup,
                    popupSuccess: !!popup 
  ...
  test.describe('红线综合验证', () => {
    ...
    test('红线拦截性能影响评估', async () => {
      ...
      console.log(`[RedLine] ✅ 性能影响测试: ${performanceTest.totalTime.toFixed(2)}ms / ${performanceTest.iterations}次`);
      console.log(`[RedLine] ✅ 平均拦截时间: ${performanceTest.averageTime.toFixed(2)}ms`);
    });
    ...
    test('红线配置完整性检查', async () => {
      ...
      const configCheck = await electronApp.evaluate(async ({ app }) => {
        ...
        const securityChecks = {
          ...
          allowRunningInsecureContentDisabled: !webPreferences.allowRunningInsecureContent,
